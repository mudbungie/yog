...
- **Every connection opens with a version preface** *(decided and landed,
  ...
  **repo-root `PROTOCOL` file** — one line, the integer, in yog and in every
  ...
  `src/wire/hello/version.rs` (the exact era, 4 → 18, closed in
  `version/exact.rs`) and the subsections below recording each bump as it was
  taken; this paragraph carried a literal `1` until bl-2410 and was five
  versions stale by then, which is what a second home for one fact does.
  **Since bl-e598 the number is a MAJOR** (§3.2): it moves on a breaking
  change only, and an addition is an edition the corpus ledger stamps.
  ...
  **What bumps it** (as amended by §3.2). A new `Query`, a new `Action`, a
  new reply kind, a new field a reader may ignore and a new word a reader may
  render as unknown are **not** a bump: they are editions, stamped in the
  corpus ledger. The integer moves when the *existing* shape breaks a reader —
  a field removed or re-typed, the framing or the envelope, what a spelling
  already in use is taken to say, or a field the engine newly requires on a
  request. Until bl-e598 every wire-visible shape change bumped it, and the
  ledger of that era is the evidence for why it stopped.
- **This document is the protocol authority** (bl-a670, with §12's split). All
  ...
  execution corpus.
- **The vocabulary ships as a conformance corpus; an addition to a
  wire-visible shape is an edition the corpus stamps, and a removal or a
  re-type bumps the protocol version** *(decided and landed, bl-32cb —
  `corpus/`, generated by `src/boundary/corpus.rs`; the addition/removal split
  is bl-e598, §3.2)*. Four components
  ...
  **The layout**: `corpus/request/<op>.json` and `corpus/reply/<kind>.json`,
  ...
  byte what a frame carries, with no wrapper — and stamped with the major the
  corpus is for. `corpus/shapes.json` is the standing record: per shape, every
  field path with the **edition** at which it appeared, plus the major, its
  `floor`, and what is `deprecated` (§3.2). Keys sorted, one trailing newline,
  nothing derived from a clock or an address: regenerating on an unchanged
  boundary is byte-identical.

  **The rule, and where it is mechanical** (§3.2). A field *gained* is
  stamped the next edition and bumps nothing; a field renamed, re-typed or
  lost, or a spelling in use withdrawn, bumps `PROTOCOL`. What the corpus adds
  is enforcement. A test regenerates and diffs, so a boundary change that
  alters an emitted byte fails until the corpus is regenerated; and because
  regenerating would otherwise erase the evidence, the standing record
  remembers every path each shape spelled, and `make corpus` **refuses** a
  path that vanished or a key that started spelling a second type unless a
  major bump is in flight and, for a loss, the path is in `corpus::DEPRECATED`.
  Both failures name both halves of the remedy. A *new* shape is an addition
  like any other: stamped, and greyable by a seat whose engine has not got it.
  (Until bl-e598 the record refused *any* moved signature at or below the
  published version — §9.9, §9.11 — which is the discipline §3.2 closed.)

  **What a client owes it**: decode every frame in both directories into its
  own types, round-trip what it emits — decode then re-encode must return the
  frame exactly — and replay the reply half at every edition from the floor
  up, and under an unknown word, refusing nothing (§3.2's conformance test). A client that only sends requests still decodes the
...
### 3.2 The number is a major, an addition is an edition, a reader grows (bl-e598)

**The ruling** (operator, 2026-09-08): the protocol kept advancing — `PROTOCOL`
went 13 → 18 in one week — and the ask was to get forward-looking and
*stabilize around something rational rather than keep iterating*. The ledger
(`src/wire/hello/version/exact.rs`) is the evidence: of the five bumps in that
week, four carried nothing but additions — an optional `address`, a
`last_seen`, a `tools` window, a `client`, two names, a `held`, two new words —
and the fifth carried one field the engine newly required on a request. Each
cost three consumer repositories a re-vendor and, under bl-bca2's hold, a
window in which no published suite composed. brazen's model wire already
lives by the alternative (`specs/canonical-protocol.md` §3.6: *within a `v`
the vocabulary only grows; additions never bump `v`*), and this section
adopts it for the seat wire.

**What the integer says now.** `PROTOCOL` is a **major-compatibility
number**. It moves only on a breaking change — a field removed or re-typed, a
frame's meaning changed under a spelling still in use, a field the engine
newly *requires* on a request — and it moved 18 → 19 here, the last bump of
the old kind and the first of the new. Everything above about the preface
stands unchanged: strict equality, fail-closed, both numbers named, no
negotiation. What changed is what a bump is *for*.

**An addition is an edition, and the ledger stamps it.** Every additive change
— a field a reader may ignore, a word a reader may render as unknown, an op, a
reply kind — ships with no bump. `corpus/shapes.json` now stamps **every field
path** with the *edition* at which it appeared: the old integer line
continued, so the stamps 1..18 a consumer already vendors keep their meaning.
The corpus's edition is its newest stamp, computed and never stored. `make
corpus` stamps a gained path with the next edition and refuses a path that
vanished or a key that started spelling a second JSON type unless a major bump
is in flight (`PROTOCOL` above `PROTOCOL_PUBLISHED`) — and, for a loss, unless
the path was deprecated first (below). A new *word* is invisible to the
ledger by design: it is additive, and the reader's catch-all is what makes it
so.

**The floor.** `shapes.json` records `floor`: the edition at which the current
major was cut (18). Every path stamped at or below it is present on every
engine speaking this major and a reader may require it, as every reader does
today; every path stamped above it is **optional to read**, because an engine
of an older edition does not write it. The floor moves in the one
regeneration that raises `PROTOCOL` and never after, so a field added in the
same unreleased wave as a bump lands post-floor — over-cautious, and safe in
the only direction that matters.

**What a reader owes — the grows-only rule.** Four obligations, and the three
consumers already meet the first structurally (every decoder indexes by key;
none enumerates an object):

1. **An unknown key is ignored.** In the hello, in a reply, in a request the
   engine reads.
2. **An absent post-floor key reads as its default**, and the default is *the
   fact before the field existed*. On a request that makes an old engine
   ignoring the key and a new engine defaulting it one act, which is what lets
   a seat send it without asking first. A field the engine *requires* on a
   request is therefore a major, and the one such field taken this week —
   18's `scope`, required rather than defaulting to `call` "so two ends cannot
   disagree about how wide the instruction was" — is under this rule spelled
   optional-defaults-to-`call`: the narrowest reading is the default, and
   *wider* is still the reading nobody reaches by accident.
3. **An unknown word maps to a named catch-all rendered honestly** —
   `Unknown(word)`, painted as *unknown state* with the word beside it —
   never to the nearest known word and never to a refusal of the row it sits
   in. lernie's `Unknown(String)` arms are the model; yog-android's strict
   `pick` on `framing`, `state` and the rest is the reader this rule changes.
4. **The reply `kind` and the request `op` stay strict.** A reader asks only
   what it paints, so an unknown kind arrives only when an existing ask
   started answering differently — a major by definition; and the engine
   refuses what it cannot act on, by name, in band, as it always has. *The
   engine refuses what it cannot act on; a reader tolerates what it cannot
   render.*

**The conformance test each consumer owes** (the child ball in each
repository carries this contract verbatim: thrall bl-6fcf, lernie bl-aa01,
yog-android bl-93cd). Beside decoding every fixture as
today:

- **Projection replay.** For every reply shape and every edition `e` from
  `floor` to the corpus's edition, delete from each frame every key whose
  stamp exceeds `e` and decode: nothing refuses. That is *replay at every
  recorded edition* with no archive of old fixtures — the stamps are the
  history, and one record projects to any point of it.
- **Word mutation.** For every string-typed path in a reply shape other than
  `kind`, replace the value in one frame with a token no build has heard of
  and decode: nothing refuses. Free text passes trivially; a vocabulary passes
  only through its catch-all.
- **The parity ledger records what the client does not RENDER, never what it
  refuses.** A fixture class that holds valid frames of kinds a seat does not
...
rarely and deliberately. A major bump is a **design ball with a migration
note** — every path removed or re-typed, the release its deprecation shipped
in, and what each reader changes — and the ordering above (consumers' mains
first, then yog, then consumers) is its checklist.

**The window rule.** A field or a shape is removed only at a major, only after
being listed in `corpus::DEPRECATED` — rendered into `shapes.json` as
`deprecated`, where a consumer reads it — in a **published** yog release, and
**never within three yog releases** of that listing shipping. The ledger
enforces the first two mechanically (a vanished path not listed is refused
even across a bump); the third is the bump ball's checklist, checked against
the tags.

**What this does not solve, named.** A meaning that changes under an
unchanged spelling (5's `oid`, 10's follow-class `attention`) is as invisible
to the ledger as it always was and is a major by rule, caught by the author.
A new request-side *word* the engine has not heard of cannot be greyed — the
seat sends it and renders the in-band refusal, which names it. Editions are
stamped per regeneration rather than per wave, so two lanes in one wave take
two numbers; harmless, since an edition holds no release. And
`PROTOCOL_PUBLISHED` keeps its residual (§9.11 amended), read far more rarely
now that a gain never consults it.

**Rejected.** Negotiation and a version list, for §3's standing reason. A
per-shape `since` map in the hello — 124 numbers per connection to say what
one integer plus the ledger the seat already vendors says. The crate version
as the edition clock — unknowable at generation time, since release-plz bumps
it after. Resetting `PROTOCOL` to 1 at the transition — the consumers'
strictly-greater gates and `PROTOCOL_PUBLISHED` read one monotonic line, and
19 costs nothing. And leaving the number at 18 with a new meaning — the hold
on 19 is what guarantees no engine of the new discipline meets a reader of
the old.
...
**Superseded in turn by §3.2 (bl-e598)**: a gain no longer consults the floor
at all — it is stamped an edition and bumps nothing — and the floor licenses
only a removal or a re-type, beside a deprecation. The residual named below
stands, read far more rarely.
...
### 9.23 The wire says what it cost, and takes the rate (bl-d13d)
...
**Why this ruling is numbered after the two that landed after it.** It landed
the same evening as §9.20 and both took that number. By the time the collision
was found, lernie's and yog-android's own `docs/DESIGN.md` cited §9.20, §9.21
and §9.22 by number — a section number is an ADDRESS other repositories hold,
and there are four of them (§12). So the duplicate resolves by giving this one
the next free number rather than by shifting two sections that are already
cited from three other trees. The chronology is in the `PROTOCOL` numbers this
...
**The observed port is a separate fact from the local one and presence must
carry the observed one** (§13.8): port preservation is best-effort even on a
NAT that has it, and a second host behind the same NAT already holding that
external port is enough to make the two differ — measured.
...
STUN servers across five destination addresses and three destination ports,
one external port every time — and its TCP mapping is endpoint-independent
too, measured with three *concurrent* held TCP connections from a single bound
source port to three distinct servers, twice, once from each host. Both
protocols preserve the port when the external port is free, so the fixed punch
port §13.2 publishes is the port a peer sees. No port-mapping protocol is
offered (UPnP, NAT-PMP and PCP all absent) and none is needed.

**Two negative facts about it, both load-bearing.** It does not hairpin: a
synchronized TCP simultaneous open between the two hosts through the public
address, retried for twenty seconds, connected in neither direction, and a UDP
trial over fifteen seconds was silent too. Two devices behind one NAT
therefore reach each other only by §13.4's direct-address rung, never by a
punch — which is what that rung is for. And its filtering is **not**
endpoint-independent: with an outbound TCP mapping held open from a port, three
independent third-party fetchers aimed at that port from addresses the mapping
had never seen all timed out while a listener bound to it accepted nothing (the
host's own firewall is not the cause — its input policy accepts and only the
