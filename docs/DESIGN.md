...
| `src/ui_state/{mod,json,knobs,fields}.rs` | the `ui.json` schema, write-through save, echo-hash, seen API, the knobs + `zoom` + the §6 escalation's `notify_unfocused` (§4.1); the per-field accessors derived from it; the crate's one ISO 8601 formatter (`format_iso8601`/`iso8601_extended`, ported calendar math, no `chrono`/`time` dep — bl-61db) |
