...
| `scripts/drive/beats_s13w.sh` | **S13's WRITE half** (bl-dbde), fired from `run_headless` after the armed rung has disbanded, for the reason the armed rung goes last: it moves the board. The board rung beside it proves yog READS balls' priority, parent and blocker graph; this proves a seatless seat can WRITE them — one `/create` carrying all four scheduling facts, the board read back for the priority, the parent and balls' own `blocked` verdict on a live edge, `bl show` as the witness for the tag no reply carries, and the clearing half (`--no-priority --no-parent --no-needs`), without which a remote coordinator could add a mis-wired blocker and never unwire it. Real substrate throughout: `blocked` is balls' resolution against the live set, not a stored field yog could echo |
