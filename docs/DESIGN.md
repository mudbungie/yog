...
| `src/dht/krpc.rs` | KRPC (BEP 5), the client's half only: the query datagram, the reply and error read back, compact `nodes`/`nodes6` parsing with a ragged tail dropped, and the 160-bit `NodeId` with its XOR distance. A query arriving is `None` — this end answers nobody |
| `src/dht/lookup.rs` | the iterative walk: α queries out, one bounded round of answers in, closer nodes learned, repeat until the K closest known have all been asked. Bounded twice against a hostile commons — a round ends at its deadline whatever is pending, the walk at `max_queries` however many closer nodes the answers invent. `ask`/`collect` are the two halves `items` reuses for the `put` round |
...
| `src/wire/rendezvous/cycle.rs` | what the thread runs: two deadlines on the clock, and what each does when due. Publish seals the punch port at the advertised addresses, signs it under the rendezvous key with a sequence that always moves, and `put`s it; poll `get`s the inbox under the derived key, opens the seal, and punches a call whose nonce is new — on its own thread, every stream that lands handed to the listener's `serve` in its held mode. The DHT client is built on first use once the bootstrap resolves, so an engine that boots offline tries again at the next deadline rather than never. Every outcome is a counter and nothing prints |
