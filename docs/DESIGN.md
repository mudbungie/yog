...
| `src/dht/items.rs` | the two BEP 44 verbs over the walk: `get` is the newest item under a key and salt that VERIFIES — an unsigned or forged item is nothing, never a value — and `put` is the same `get` walk (it is the walk that collects write tokens) followed by one round of `put` to the K closest token holders, answering how many acknowledged, with zero an error naming the first refusal |
...
| `src/dht/lookup.rs` | the iterative walk: α queries out, one bounded round of answers in, closer nodes learned, repeat until the K closest known have all been asked. Bounded twice against a hostile commons — a round ends at its deadline whatever is pending, the walk at `max_queries` however many closer nodes the answers invent. `ask`/`collect` are the two halves `items` reuses for the `put` round. The walk keeps each answering node's `ip` claim for `Dht::observed`; a walk replaces the last one's claims, so a walk that heard nobody observes nothing |
