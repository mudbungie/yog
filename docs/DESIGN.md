...
is. **Both answer through the same carrier since bl-dc3f**: `Query::ReadConfig`
returns the raw `text` and a `settings` array beside it — this file's two rows
with `context_window`'s bounds on its control, that file's empty list — so the
difference between a typed destination and a fallback is a value in one answer
rather than two shapes a seat must tell apart.
