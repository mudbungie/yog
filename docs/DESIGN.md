...
- **Actions** are the ops trail's rows (§4.2): everything that mutates a
  ...
  scan, close, assign, release, move, create, update),
  ...
  pinned target) and `Retire` (release one, discarding its ref only when
  retention says so). It folds for the monitor's reasons exactly, and on a seam
  every layer beneath the roster already drew — `boundary::fan` holds both
  executors, `codec::fan` both spellings, `line::fan` both readers, and each
  file's own doc says "the fan's two". The fold is what made room for
...
| `src/fan/{mod,cohort,retention}.rs` | the §3.8 **mutating fan** (VISION §4.10, bl-8746): `mod` the family's own `Verb` — the two gestures the boundary carries as one `Action::Fan`, folded there in bl-a33d beside `monitor::Verb` and `fleet::Verb` and for their reasons — the obligation, the candidates and the spread — a thin in-process consumer of balls 0.5.10's attempt capability, which owns every name and path here, plus the proof that one fan's members share one base; `cohort` the membership fold, derived from yog's own fire rows and never from the agent-writable `cd` mark; `retention` the one severable `cadence.yaml` policy that turns a released candidate into a discarded one, absent meaning never discard |
