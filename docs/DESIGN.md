...
| `src/dht/lookup.rs` | the iterative walk: α queries out, one bounded round of answers in, closer nodes learned, repeat until the K closest known have all been asked. Bounded twice against a hostile commons — a round ends at its deadline whatever is pending, the walk at `max_queries` however many closer nodes the answers invent. The bootstrap round always asks `find_node` and every later round the walk's own verb, so the BEP 44 verbs reach nodes near the target rather than the routers, which never answer `get` (bl-f6e1). The bootstrap is a door, never a result (bl-9408): its answers seed the pool and its claims vote, but nothing it says is in the outcome, so a walk whose learned nodes were all silent is the dark-commons `Err`, and the pool keys a node by `(distance, address)` so a repeated entry is learned once and one id at two addresses is two nodes. `ask`/`collect` are the two halves `items` reuses for the `put` round. The walk keeps each answering node's `ip` claim for `Dht::observed`; a walk replaces the last one's claims, so a walk that heard nobody observes nothing |
