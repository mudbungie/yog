...
**Where the prediction went: the seat** (bl-7cc8). `start::goal::preview`,
its `Composer` view-model and `identity_preview` are **deleted**. They backed a
greyed `will be named <name>` above a composer, `Prepared` carries no predicted
name for a reply to hand back, and no §8.5 answer states one — so the
derivation had no caller and no carrier, which is 100%-covered code proving
nothing about a running system. A seat that wants a prediction files against
yog and the reply is designed then; until then the wording of a prediction is
the seat's, like every other view. The seed-continuity rulings that governed
them (bl-28ba: a seed lives as long as the prediction it backs; bl-dd3d: the
successor comes off the spent seed's own stream, one `SplitMix64` step, so
entropy enters once and a known opening seed determines the whole run of names)
were **facts about a preview→fire pair this crate no longer has either half
of**, and they go with the preview. What survives here unconditionally is the
reason those rulings existed:
...
- **The directory is a birth parameter, not part of the message** (bl-7927,
  ...
  no default for it and holds no box. The `ActionsState` that once held that
  box's value — with the composer drafts keyed by target and the selection they
  were typed against — is **deleted** (bl-7cc8): a running frontend's RAM kept
  in a server no §8.5 act ever asked it for. The field's own pre-flight refusal
  (`actions::work_dir_refusal`) went the same way (bl-33e9): it asked the *spawn
  boundary's* question one step early on behalf of a box this crate no longer
  holds, so the one home for it is the executor's own refusal below.
- **The birth ROLE is a field of the gesture too, and it is the one field of a
  `Prepared` a seat is expected to change** (bl-9ced, litany 0.0.12 upstream
  bl-946c). `Prepared.role` is the `souls/<role>.md`, the `providers.yaml`
  assignment and the `tools:` grant litany resolves out of the **same** config
  commit `Prepared.lineage` chose, passed at fire as `--role`; `None` is
  litany's `worker`, an absent flag rather than a word yog spells, so an
  unroled start is byte-identical to every start before this. **yog derives
  nothing into it** — a lineage falls out of the ball's tags (§8.7), and which
  role an operator wants their *own* conversation under falls out of nothing —
  so `/prepare` answers `null` and a seat that wants a planning conversation
  deposits the same body back as `/prompt` with `"role": "planner"`. That is
  plan mode entered in one act, where before it meant authoring a lineage per
  workspace whose two edits only restated what the lineage already carried.
  Two consequences worth stating: a role the governing commit does not declare
  is refused by **litany's** resolution before the fork, so a bad value leaves
  no branch and yog holds no second validity check; and the §8.3 sign-in door
  now reads the fired role's `roles.<r>.provider` rather than `worker`'s, in
  both directions — a `planner` start whose row is empty is refused even when
  `worker`'s is signed in, and the refusal names `/model planner …`, because a
  remedy pointing at `worker` would fix a row the start never resolves.
  Accepting a plan — litany's `retarget --role worker` — is **not** here: `/retarget`
  is still the bare verb (§9.4), and the return half is named in REMOTE §9.21
  rather than shipped.
...
- **A birth is one act or none** (bl-1af5). `litany new` makes the directory,
  then the bare repository, then the first `config/default` commit, and a
  failure at the third left `<workspace>/repo.git` behind. That debris **is** a
  workspace by §3.1's definition, so it enumerates — and the client whose
  failed reply carried no registration (REMOTE §4) can then neither address the
  name (`unknown workspace`) nor found past it (the raise can found or resume,
  never join another client's wall), while `workspaces` answers an empty list
  about it. The name is unusable forever, the state that causes it is invisible
  to every read the interface offers, and `rm -rf` is the only exit. It landed
  on `home`, the one name §3.1's bootstrap types without asking, so it was the
  first name a first-time user could lose. bl-c9d2's raise already resumes past
  a directory with **no** marker; this is the same wedge one step later, where
  no raise can help.
  ...
  once litany has finished — a same-directory rename, atomic and never EXDEV.
  A failure at any step leaves no workspace: the temp is removed on the way
  out, and even a process killed mid-birth leaves only a dot-named directory
  that no enumeration sees (`binding` skips I3 temps) and that blocks no name,
  where the old shape left the name wedged. It is the same posture the rest of
  the start flow takes about spending nothing on a refusal, and it dissolves
  the class rather than documenting one cause. Three consequences, each
  deliberate: the §4.2 trail's `new` row names the **temp**, because that is
  the path litany was actually given; debris already at the destination is
  renamed **over** only when the directory is empty, which is exactly bl-c9d2's
  case, a non-empty one earning `ENOTEMPTY` where litany used to say
  `destination is not empty`; and the I3 sweep does not collect a leftover
  birth, because it removes files and never directories — an inert
...
- **The table and the ceiling are read and set through the boundary.** §4.1
  ...
  `/ceiling off`. The wire shapes are REMOTE §9.24's.
...
dialog gates, whichever frontend fires). The §11 seats that reached it —
`nav::menu`'s roster and `shell::delete_agent` — are a seat's and are gone from
this crate (bl-7942, bl-33e9).
...
Two derivations bl-77bc wrote for the window had **no carrier and no caller**,
and bl-7cc8 **deleted** them: `science::respdiff` (V3.3's line LCS over two
candidates' terminal responses) and `science::compose` (the four group
affordances as composed dispatch text). The reply carries each row's `response`
string, so a seat that wants a comparison already holds both sides, and a diff
computed here would be exactly the second implementation this crate exists to
prevent; composed dispatch text is a composer's, and no reply carries any. **The
V3.3 comparison and the four affordances are the seat's**, and this document
states the affordances' *meaning* (Judge, Synthesize, Deliver, Retire above) —
never their wording, which no yog act produces.
...
- **The operator's own two lines (bl-c417).** Dismissing an alarm and ending a
  ...
  - `["yog-step","ack-failures"]` — the **ack**, a *global seen-watermark*. Every
    ...
    the §7.3 standing each answered row carries (`opslog::standings`, whose
    `acked` word IS this watermark read) and the ⚠
    ...
    has been seen. One line shape, no per-origin variants — dismissing from anywhere
...
- **The capability boundary's two row shapes (§8.6, bl-765d/bl-94b4).** Also
  ...
  ["yog-control","answer",<key>,"pass"|"hold"|"refuse",<scope>]`, `stdout` the
  control's own reason off the hold mark. The `<scope>` is `call` (the key is
  the held `tool_use` id — the shape every answer had before bl-94a5, and a
  four-word row with no scope word still reads as one), `conversation` (the key
  is `<agent> <tool>@<class>`) or `workspace` (the key is `<tool>@<class>`, and
  the workspace is the row's own `cwd` rather than a second copy of it in the
  argv). A **floor** over one conversation is
...
- **The badge/rollup half owes the same honesty (bl-8433), and since bl-4d81
  it is what CROSSES.** `Query::Ops` answers each row with `failed`,
  `exit_label` and `standing` beside the durable line — the last being
  `opslog::Standing`, this rollup folded with the ack watermark below, total
  over every row (`clean`/`detached`/`live`/`retired`/`acked`). So the words a
  seat paints are yog's, the retirement rule is asked once, and §7.3's banner is
  a filter rather than a re-derivation. `badge::op_badge` stays the glyph-and-
  phrase mapping over `OpOutcome` for the counts this process folds itself; the
  standing is its wire-side sibling with the one fact `OpOutcome` cannot carry.
  bl-afa9 fixed the
...
| 35 | **How full a conversation's context is** — the percentage `Reply::Agent`'s settings rows carry per conversation (bl-a48b) | the **last `Usage` line** of the root agent's **latest** `steps/<root>/<NNN>/response.json`, the model that step's `request.json` names (#16's walk), and the `context_window` **that same usage line carries** — brazen stamps the resolved model row's window on every `Usage` event (its model-discovery §5.5, 0.0.9) | `context::of_agent`, a pure filter over `Snapshot::bills` — no second disk pass, no stored counter, nothing cached. **Fullness is not spend, and the difference is load-bearing:** #16 folds every attempt of every step of a whole descent (what exhausts `max_total_tokens`, and what keeps growing after a compaction empties the context), while this is one number off ONE step — so the walk carries two extra columns, the step's own `seq` (making "which is the latest" an in-memory question, exactly as bl-9dd4 made scope one) and its **last** attempt segment's counters (a step retried three times must not read as a context three times its size). **One agent's own latest step, never a descent's** — a dispatched child runs its own context in its own tree, so the subject is the agent the read NAMES (bl-131d). It named the root instead until then, which contradicted this very sentence: a child selected in §11 was answered with its parent's fullness, off a filter one argument away from the honest number. **The prompt is one formula with one home since bl-6621** (`BudgetSpend::prompt_tokens`), which #16's spend fold reads as well — this row stated a rule while the spend fold summed all four counters beside it, and that divergence was the double-count. **Since brazen 0.0.10 the formula is a read**: `input_total_tokens` is the whole prompt with the cached slices inside, sealed by the decoder that knows the provider's containment shape, so `max(input, cache_read + cache_write)` returns it unchanged and decides nothing (bl-bf8b). The max is the **fallback for a line written before that counter existed**, and it is a fallback per LINE rather than per record — the question is asked once per usage event, so a `response.json` spanning a `bz` upgrade folds each segment by its own shape with no version detected anywhere. brazen's canonical `Usage` was deliberately unnormalized about overlap because its providers disagree — Anthropic reports the three as **disjoint** slices of one prompt while OpenAI's `prompt_tokens` and Google's `promptTokenCount` already **contain** the cached slice beside them — so summing over-states one shape and taking `input_tokens` alone under-states the other by nearly everything (brazen marks Anthropic prompts for caching unconditionally). Under the fallback the maximum is exact where the slice is contained, degrades to plain `input_tokens` where no cache counters are reported at all, and is a **floor** where they are disjoint; it never over-states. Normalizing that overlap was brazen's job rather than yog's to guess at, and brazen did it — **which retires the exit `total_tokens` used to name**: "collapses back to the plain four-counter sum" is now the wrong shape, because the served total already holds the cached slice and a sum would bill it twice. The collapsed form is what ships, `prompt + output`. **The denominator is the window the usage line itself states (bl-9c8a), and yog holds no table of windows.** It was a hand declaration in `models.yaml` — yog's own one-fact block inside litany's file, authored by a Declare control, edited by §9.5 typed rows, adopted onto the snapshot on the 15 s sweep — and that whole family is deleted, for one reason with three faces. *Single source of truth:* litany's `window_percent` compaction trigger (its bl-a537) divides by exactly this in-band number off the same step record and keeps no per-model table, so a window yog declared on its own was a second representation of one fact that the engine actually compacting the context could never see — the percentage here and the compaction one layer down could disagree about one context, which is worse than no figure. *The general path:* the ball this ruling closes had designed a read-time query over brazen's model cache (`model_cache_at`, per workspace, per provider row, folded on the sweep, with a precedence rule over the declaration); brazen performs that very read on every call and writes the answer on the event the engine already records per step, so the query, its keying, the sweep arm and the precedence question all dissolve into `budgets::context_window` — the last window the step's usage lines state, by the same per-field last-wins rule the numerator is read by. *Severability:* the one seat to state a window for a row whose list serves none is upstream, on brazen's provider row, where for Ollama it is ALREADY the `options.num_ctx` in force since brazen 0.0.10 (bl-f19d) — declared once, reaching the request, the event, litany and yog alike, so the number in force and the denominator cannot diverge. **The ball filed from bl-9c8a for the other rows landed** (brazen bl-c655, 0.0.13, pinned by bl-d58d): a `[[provider]]` row's `context_windows = { "<wire-model-id>" = <limit> }` is the last rung of a three-rung ladder brazen stamps on the event — what the request pins, else what the list served, else what the row declares — and a consumer never learns which rung answered, so nothing here changes to read it. So EVERY row can now render a figure, on one line of the operator's own brazen config; a row nobody has written that line for still renders none, which is the state a fresh world starts in (bl-d9cb) with the remedy now in the right program and reachable from §9.4. **The numerator is `input_total_tokens` where the line carries it** (brazen 0.0.10, bl-d192: the whole prompt, sealed by the decoder that knows the provider's containment shape) and the `max(input, cache_read + cache_write)` reading of `BudgetSpend::prompt_tokens` on a line written before that counter existed — not a version shim but the two shapes a step record on disk has forever; the prompt reading's whole why is in `budgets`. **Per field, last wins:** brazen's usage events are partial (Anthropic's `message_start` carries the prompt, its `message_delta` only the output), and reading the final line alone rendered that shape as a prompt of zero. **No window, no figure** (no step, no model on the step, no window stated on its usage lines or a zero one): the row is absent, never a percentage of a default — and a row that serves no window is the ordinary state on most rows rather than a corner — the same no-capability-theater rule §3.5's unpriced remainder keeps. |
...
**The code that held them is gone too** (bl-7cc8). Until that ball the *types*
for a face's ephemera outlived the face: composer drafts keyed by their target
(`actions::Drafts`), the `ActionsState` holding them beside the selection and
the birth-config box, and the predicate that decided when a clean send cleared
one. They were reached by nothing but their own tests, and no §8.5 act asks for
or answers any of them, so they were a seat's RAM declared in a server. A
whitelist that names what may be held here is falsified by held state nothing
...
2. State **at rest** (Quiescent or Stopped), no `refs/litany/abandoned/<id>`,
   **nobody dispatched it**, and tip oid ≠ `seen[ws][agent].stopped` — a
   ...
   and a rest whose latest turn was **cut off at the request's output cap** says
   `truncated` *(bl-ebef)*. The two refinements are mutually exclusive by
   construction — a truncation frames COMPLETE around a `finish` whose reason is
   `length`, so it carries no failure sentence, and a refusal carries no
   `finish` to read a reason off — and neither adds a firing. litany 0.0.11
   makes the cut a named failure (`Error::OutputTruncated`, upstream
   bl-155f/bl-ecf9): the staging sink is never sealed, so **nothing is
   committed and no tool call is run**, which is exactly what *"came to rest —
   your turn"* denied. The remedy is the role's `max_output_tokens:` in
...
**Attention escalates to the desktop, and the escalating is a seat's
(bl-e160, ruled bl-09ef).** §6 is yog's core promise — *does anything need me?*
— and it answers only where somebody is reading an answer, which is precisely
not when it matters. So when a new thing needs the operator and no seat is
being looked at, the desktop is told: one notification per conversation, naming
its workspace wall, its §3.3 display name, and the firing rules in words.

**Whose desktop is not a question yog may answer, so yog does not raise it.** A
notification belongs on the box the operator is looking at, which is wherever
the *seat* is (REMOTE §12) and never the engine box — a headless engine on
another machine announcing to its own session tells nobody. `src/alert/`
derived the announcement here and spawned `notify-send` here, with no caller
since the frame left; it is **deleted** (bl-09ef), and with it the §4.1
`notify_unfocused` key, which is a fact about one pane of glass and therefore a
seat's own to store.

*Nothing new is modelled, and one wording crosses.* An alert is a projection of
the §8.5 decision queue (`boundary::answer::queue`) — already "what needs you"
made addressable — so the number a seat states and the ask a desktop names
cannot diverge, and the acknowledgement that empties one empties the other. The
sentence stays yog's: **`AttentionKind::says` is the one home for each rule in
words, and it crosses on the queue row's `says` key** beside the signal tokens
(`boundary::reply::queue`), so a seat announces with the engine's wording
rather than inventing a second one — the drift this document exists to stop.
**No sixth signal, no ref, no `ui.json` `seen` key, no verb.** An announcement
is *output*: it writes nothing, so I7 stands and `ops.jsonl` (§4.2, the log of
what yog did to the world) gains no row — including on failure.
...
*The baseline is the seat's, and yog hands it the arrival signal.* What has been
announced is a **difference against the queue as this seat last saw it** — never
a stored fact, because two seats each own theirs and neither should converge.
The difference is cheap because the far side is handed changes rather than
polling for them: **the attention lane** (REMOTE §14.1, bl-09aa,
...
mistaken for news. Two rules fall out and both are load-bearing: a seat that has
just opened has witnessed no *arrival*, so its first frame is its baseline and
says nothing (the general path with no prior observation, not a first-run
branch); and **an unanswered ask is not a reading of the queue**, so a dropped
lane advances nothing — folding an empty answer would read as everything
departing and then, on the next frame, as everything arriving at once, which is
the first-boot flood re-armed at the ask cadence.

*The mechanism at the seat is a spawned binary, and had to be.* AGENTS.md rule
6 forbids a new dependency, and every Rust crate that raises a freedesktop
notification (`notify-rust` and its `zbus`/`dbus` stack) is one. The spec's own
reference client is a binary every desktop already ships — libnotify's
`notify-send` — and this suite's substrate is spawned binaries, so it is the
discipline already in hand pointed one process further out. It is spawned
**bare** (the notifier is the operator's desktop session reached through the
session-bus address the process inherited, not substrate, so the §16.2 world
fold must not ride it), nothing waits on it, and every failure — an absent
notifier included — is **silent**: a desktop that cannot take a notification is
not an event, and the feature degrades to the world before it existed. All of
that is now the seat crate's to hold, and this paragraph records the ruling it
inherits rather than a mechanism standing here.

*Severable at the seat, in the same one key.* `notify_unfocused` is the switch —
absent, the escalation is armed, because the attention answer is invisible
exactly when the operator is not reading it and a notifier that is off until you
find its switch is a feature nobody has. It is a **glass** fact: whether *this*
pane may raise a notification is that pane's question, so it lives wherever that
seat keeps its own local state (bl-f936) and never in a yog document.
...
surface the operator is reading instead of the steps query nobody opens on a
conversation that looks stopped.

Three things follow from making it the wound rather than a refusal notice:

- **The affordance stops being a second fact.** `auth_failed` was a stored
  field beside the wound and a boolean beside `auth_row` on the wire — one
  answer in three places. It is now a query over the wound
  (`StepSummary::auth_failed`), the wire spells the class once, and the §8.5
...
- **The notice is folded, never built.** It rides `Query::Transcript`'s answer
  ...
  derivation and a healthy in-flight conversation pays for no steps walk.
...
- **The three periods are settings, not constants (bl-3381).** The 100 ms
  ...
  follows the tuning without any reader touching the file. **Every one of them
  is spent on this side, and none crosses §8.5** (bl-776a): `late_pass` judges
  this engine's own passes, `stale_after` decides the word the workspaces answer
  already carries, and the wound grace is waited out before a wound is stated at
  all (§7.3). That is the whole of what a seat needs — a period a seat had to
  honour would be one derivation with two implementations, re-parsed out of a
  YAML file `Query::ReadConfig` hands over as raw text, and a `cadence.yaml`
  edit would silently falsify the copy. The parse is total: a
...
| yog crash between a gesture claim and its reply | the claim's file lock dies with the claimant (§8.5, bl-d1f1); the next engine boot answers the reply slot **in doubt** — the depositor's poll terminates with the refusal, and the recovery is a read of the world, never a re-send |
...
| Failed action (short verb or start step) | **an answered fact, never stderr-only, and said exactly once**: the full `ops.jsonl` entry (argv, cwd, exit, origin, stderr) rides `Query::Ops`, *and* the **originating surface** — and only it — is the one place it is raised as an alarm. *A **failure** is `OpRow::failed`, which for a detached `-2` row is a sink the derivation folded in because the launch produced nothing (§4.2, bl-b95e) — a driver that filed a notice and carried on is a handoff and reaches no alarm at all.* **The carrier is the ops row itself** (bl-4d81): each row of the `Query::Ops` answer carries `failed`, `exit_label` — the sentinel and `128+n` readings in words, so no seat paints a bare `-3` — and `standing`, §6's retirement folded with the §4.2 ack watermark (`clean`/`detached`/`live`/`retired`/`acked`, `opslog::standing`). **A banner is therefore the rows standing `live`, grouped by `origin`** — a seat renders and never classifies, which is the whole of the rule: the sentinel table, the `(cwd, verb)` retirement key and the ack scan have one home and a seat that re-derived them could diverge silently rather than fail to compile. It is **derived from the refreshed ops tail on every read**, never cached at dispatch: the dispatch handler runs microseconds after a detached spawn, before the child can die, so a verdict taken there is clean forever and the sink row above surfaces to nobody (bl-4895 — three live prompts, three populated sinks, zero banners). No error path may exist that only writes to stderr (STORIES INV-2) — since bl-7942 that is structural rather than disciplined, this process having no terminal to write one to. **The originating surface is the op's `origin` field (§4.2), stamped at dispatch — see the row below for the rule it obeys.** *What this retired: `AppModel::last_failure(origin)` and `opslog::SurfaceFailure`, the per-surface question and its compact argv-plus-stderr-tail projection. Both were the seat's half of a derivation that now crosses whole, both had lost their production caller with the window (bl-7942), and everything `SurfaceFailure` projected is on the row it projected from.* |
| **How an alarm ends** (bl-c417) | Two ways, and until bl-c417 only one existed. (1) **Retirement**: a newer op of the same `origin` that did not fail — the §6 rule, per-origin (the row below). (2) **The operator's ack**: the projection runs over `opslog::since_ack`'s rows, so the §4.2 ack line quiets every origin's alarm at once, whether or not anything was retried. **Both endings are one answered field** since bl-4d81 — a row's `standing` is `retired` for the first and `acked` for the second, so the two exits are told apart on the wire rather than collapsing into "no longer live", and a seat can say *why* an alarm is down. That second exit is the whole of the complaint — *"I need a way to make the failed notification go away"* — because an operator who reads an error and decides **not** to retry could not otherwise put it down: the only exit was a *successful re-run of the same verb*, so a failure the operator has understood and chosen to leave alone stayed raised forever. The ack is not a flag on a control: it is a durable line, so the dismissal converges to every other seat and survives a restart exactly as the failure did. It is also not amnesia — a **new** failure of that origin lands after the watermark and raises again. Its one spelling is the ops trail's own operator verb (§4.2, `opslog::operator`), so however many controls a seat offers, there is one act and one wording. |
| What "the originating surface" means (bl-48f8) | **The op's *subject*, recorded at dispatch, one alarm per subject.** Three subjects exist: a **ball** op (every `bl` verb, and every step of a ball-rung start — `bl create`/`bl claim`, but equally its `litany prime`/`litany new`/`["yog-step","mkdir"]` substrate steps and its detached `litany prompt`) is about a ball; a **conversation** op (`litany message`/`stop`/`scan`, and every step of a bare- or path-rung start) is about a conversation; a **world** op (the §9 config writes, the §16.3 space knob, the §8.3 login flow, the §3.6 unmaking, and yog's own §7.2 drift lines) is about the world, and raises **no alarm at all**, because each of those acts states its own outcome in its own reply and a config-write failure is not news anything else has business breaking into. §6's retirement is per-subject with it: a subject's alarm clears when *that subject's* next action runs clean, and never when another's does. It is the field a banner **groups by**, and the only one of §7.3's facts that could not be derived — which is why it is stored on the line and why bl-4d81 left it exactly where it was while making its four neighbours answered. **The subject is not where a hand landed:** one gesture has one body however many controls reach it, so a ball verb is about a ball wherever it was asked from — forking that body per control would record a distinction no operator makes, and since bl-7942 there are no controls in this process to fork it by. What this closes: the standing-failure query was once global over the ops tail and three surfaces raised it unconditionally, so a single failed start accused the balls section, the composer *and* the bootstrap box at once, a config-editor failure accused the composer, and any surface's clean run wiped every other's live alarm. |
...
1. Resolve the target workspace: the one the gesture names (§8.5's `workspace`
   ...
   **The pinned template already grants the complete worker pool; yog grants
   ...
   python, read_file, read_tool_output, remember, search_history]` — the entire
   shipped pool,
   ...
   **One name came back, and it is the one litany's template cannot write
   (bl-0460).** bl-52b7 made `clients` part of the **grant** rather than a
   host injection beside it — litany permits a tool that is *in the role's
   grant OR injected by the host*, so injecting beside a grant was granting,
   and a compactor with an empty grant reached a foot's `bash` through
   `clients {"op":"load"}`. The law is right and it left every workspace on
   the wrong side of it: `clients` is **yog's** tool, so litany's template
   cannot name it and no role anywhere could reach a foot. So the §8.6
   convergence authors that one name into `roles.worker.tools`, **outside the
   create skip**, which is what makes an existing workspace pick it up at its
   next start rather than needing an operator's edit. Note what this is not.
   It is not bl-7fc8's path returning: that one re-asserted `message` and
   `dispatch`, names the template already carried, and authored nothing on
   every template since. And it is never machinery — only the
   worker, the compactor's empty grant being the confinement bl-52b7 restored
   and the reviewer's list being stated as its own confinement in litany's
   template. The install-wide template override
   (`<world>/litany/template/providers.yaml`) was the obvious home and is
   unavailable in both halves: litany's overlay replaces a template file
   wholesale and the embedded original is not reachable from yog (the crate
   exports `cmd` and `mint`), so yog would carry a hand-copy of the file whose
   own header warns that a copy is drift — and it would reach only workspaces
   not yet born, while the defect is in every existing one. A worker that
   declares no `tools:` at all is left alone: an absent list is a role granted
   nothing, which is a thing an operator may mean.

   **And one name goes, because a grant is durable and a pin is not
   (bl-d281).** bl-d06c's park was `multi_tool` classified `opaque` and held
   on every call, and the guard it left binds the intrinsic map to the grant
   the *pinned* engine authors. That does nothing for the workspaces already
   on disk: litany retired `multi_tool` at 0.0.8 (its
   `docs/DESIGN_CODE_EXECUTION.md` §5), a `providers.yaml` written before that
   still names it, the grant gate passes it because it is in the role's list,
   and the conversation parks with tool calls and no reply. So the same pass
   **prunes** the worker's grant to what this install can call, and an old
   workspace fixes itself at its next start.

   The set it prunes against is derived and never written down
   (`src/start/grant/keep.rs`): `litany::cmd::BUILTIN_TOOLS`, the names
   `litany tool <name>` answers to and which the engine exports for exactly
   this reader, plus `tool_host::engine_act::NAMES` and
   `tool_host::clients::NAME` — the two granted sets that are yog's own. A pin
   bump moves it; no code is edited. `tests/engine_grant.rs` holds it against
   the grant the real binary authors, which is the direction a hand copy in
   this tree cannot answer.

   **The operator's own `rules:` row is the one thing that widens it.** A name
   outside the shipped set is still keepable, and the signal is one that
   already exists: a `capability.yaml` `rules:` row naming the name's class
   (§8.6's own policy file). That is REMOTE §5.4's worktree lane in its only
   working configuration — a bare name a registered machine advertises and
   consents to run at the conversation's cwd — because without the row every
   call of it would classify `opaque` and park exactly as the retired built-in
   did. So the prune removes precisely the names nothing can execute and
   nobody has vouched for, and an operator who means a foreign grant states
   its reach in the file that already exists to state reaches.

   **It is still not tool-name narrowing** (VISION §4.11's standing
   rejection). Narrowing removes a name whose *effects* are distrusted, and
   the answer to that is the per-invocation adjudication above, which is
   untouched. This removes a name **no invocation of which can execute**; its
   floor is the engine's whole shipped pool, so it can never take away a
   capability the model would otherwise have had.

   **A grant and its description are one fact, and writing one alone broke
   every conversation (bl-7d33).** bl-0460 asserted the grant and nothing
   else, and litany's *descriptions-always* rule (its ARCH §3.3) refuses a
   **fork** whose role grants a tool the governing config commit does not
   describe — before any injection is consulted, so being injected is no
   exception. Its own words are the argument back at it: *"`providers.yaml`
   and `descriptions/**` disagree, and both live in that one commit"*. So
   every `/prompt` on that tree answered `started` and died at the fork with
   `no descriptions/tools/clients.json`, which is the whole product. The
   convergence writes `descriptions/tools/clients.json` beside the grant now,
   in the same pass, and each file is checked for itself — so a workspace
   carrying bl-0460's half-written state converges at its next start rather
   than needing a hand. The bytes are `clients::schema()` verbatim, the very
   value the injection declares, so the committed description and the wire
   declaration cannot disagree; that also makes it honest for the second
   reader, since litany's bl-55b1 cut made `descriptions/tools/` the callable
   set an agent reads with `bash`. **The litany pin does not decide it**:
   0.0.10 and 0.0.11 run the same check with the same input, `clients` being
   yog's tool in both. And it is still one policy — no grant, no description:
   a worker that declares no `tools:` is granted nothing and described
   nothing.
...
2. Answer the **`Prepared`** (§3.3), carrying the rung's prefill as
   ...
   having none. **A blank goal never sends** (bl-6191), and the refusal is the
   **engine's**, seated in the `Prompt` door (`boundary::dispatch::doors`,
   §8.5) — the one place every spelling of the fire passes, so a line, a
   deposit and the §4.3 loop's own re-prompt are gated once between them. A
   goal empty after trimming is refused there in one sentence, **ahead of the
   §4.11 confinement read and the §3.5 ceiling** — whose refusal writes a §4.2
   row — so a fire with nothing to say touches no wall and costs no trail. A
   seat may grey its own send button as well, but that is a view: a blank goal
   reaching a model is spend for nothing and a conversation whose first entry
   is empty, which makes it the same class as the ceiling and not the same
   class as a composer's text (bl-54c1 reversing bl-33e9's ruling, which
   retired the callerless `actions::goal_present` and left the invariant with
   no enforcement anywhere). yog answers the prefill; what is typed over it is
   judged at the door it is fired through. On
...
- **The rung has one home, and it is the `Prompt` door** (bl-2291, designed on
  the seat's bl-b180). Between bl-7cc8 and bl-2291 the predicate lived in this
  paragraph and was enforced nowhere: the callerless `StartGate`/`WallCredit`
  fold was deleted, its inputs crossed as `Query::Providers`, and the closing
  line reserved the carrier for whichever seat asked. None did, because a seat
  re-judging the rows is a second implementation of a settled decision (VISION
  §8) — and there are two seats now, so it would have been a third. So
  `boundary::dispatch::signin` judges the wall at the door every spelling of
  the fire passes (`doors::prompt`, the seat bl-54c1 chose for the blank goal
  on the same reasoning), in the order **blank goal → sign in → confinement →
  ceiling**: behind the blank goal because a fire with nothing to say reads no
  wall, ahead of the other two because a fire that would reach no model must
  cost neither a policy read nor a trail row. **No new verb and no new
  shape**: the refusal is the `{"ok": false, "error": …}` every act already
  answers with, `PROTOCOL` stands, and a seat paints it where it paints every
  refusal and re-derives nothing (REMOTE §9.19). The seat-side question the
  old fold carried — *what if the fire is typed before the read is back* —
  dissolves rather than being answered: there is no read to wait on, the goal
  is posted as always, and the answer is ready or refused.
- **The predicate is brazen's own `credential` column**, the wall's effective
  table read in-process through the linked brazen (`RealBzRunner`, §16.7 W10)
  — no network and no spawn, the ruling's "must not add a round trip" kept.
  Every spelling but `missing` and `not required` readies a row — `stored`,
  `ambient`, `inline`, and any this build cannot read, because refusing a wall
  whose rows carry a credential a run would spend blocks a working setup, and
  no surface refuses on an unanswered question. `missing` is the one refusal.
- **And the row it is asked of is the one the ROLES name (bl-58e7).** For one
  round the fold asked it of the wall — *any* row credentialed readies it —
  and that is not the question a fire asks: the loop resolves
  `roles.<r>.provider`, so one credentialed row the roles do not name readies
  nothing. Reproduced on two fresh workspaces: a wall carrying exactly one
  ...
  fired — and in a multi-seat workspace the operator who finds the corpse is
  not the one who started it. So the fold asks it of the row
  `roles.<r>.provider` names, and the refusal says so.
- **The fired role and only it** — `worker` for a start that names none,
  because this door births roots and (litany:) *roots are workers*, and the
  role the gesture names otherwise (`Prepared.role`, bl-9ced). It was `worker`
  unconditionally until litany 0.0.12 made a root's role an input; a door that
  kept reading `worker` would then judge a row the fire does not resolve, in
  both directions — passing a `planner` start whose row is empty, refusing one
  whose row is signed in — and would word its remedy about `worker`, sending
  the operator to fix a row the start never touches.
  The other declared roles are not this fire's question,
  and refusing on one would be the conservatism this section warns about:
  litany's shipped template declares a `reviewer` **unbound** — nothing
  dispatches it until a workflow binds `worker_flush: dispatch(reviewer)` — so
  a wall that would run for hours would be refused over a role that never
  fires. A `compactor` pointing at a dead row does fail, later, at its own
  moment and through §8.3's step failure and the `failure` clause bl-d774 put
  on the row: a corpse with a cause, which is what those surfaces are for, and
  not the zero-percent birth this rung exists to stop. A lineage that declares
  no worker at all falls back to the wall predicate above, which is the general
  path with an empty input: a config that cannot be read declares nothing (the
  `Roles` read's own answer rather than a refusal), and what is left to ask is
  whether any row here could answer at all.
- **A keyless row is not a credential — unless a role names it.** The ruling's
  own wording is *"any row `stored` or `not required`"*; taken literally it is
  vacuous, because brazen merges its built-in table under **every** config, so
  `ollama` and `claude-code` read `not required` on every wall there can be
  (a wall with no config file at all answers eight rows). Nor are they what a
  doomed start was routed to: both claim no model prefixes and are reached only
  by an explicit `--provider`. So a keyless row alone does not ready a wall.
  **The clause the door forces (bl-2291):** a `not required` row that the
  fire's lineage (`Prepared.lineage`, `config/default` otherwise) names in
  `roles.<r>.provider` — where `<r>` is the fire's own role (`Prepared.role`,
  `worker` otherwise; bl-9ced) — *does* ready it. The old fold recorded its conservatism
  as tolerable — *told to sign in when their setup needs no sign-in, and the
  remedy is one sentence they can read past* — and a seat's sentence could be
  read past; a door's refusal cannot, so without the clause a keyless-only
  setup could never begin a conversation. A role pointing at `ollama` is the
  operator's own hand and not the merge, the read is the one §9.4's pick gate
  already makes at its own fire, and `missing` stays the one refusal whoever
  named the row.
- **Two states, not three** — and the refused one has **three sentences,
  because the acts are three** (bl-58e7, bl-21e9). Ready is the fire
  proceeding, untouched. Refused is one sentence, fact first and remedy last,
  and which sentence is decided by what is wrong: a role resolving a row that
  is *here and empty* is a sign-in on that row (`/login <provider>` for an
  oauth row, and for every other credential model the secret is a value in the
  wall's own `config.toml`, which is `/config brazen`); a role resolving a row
  **this wall does not declare at all** is an act on the *table* and never a
  sign-in — telling its operator to `/login` a row that is not there is a loop
...
- **A refused fire spends nothing.** The gesture is refused whole, so the
  caller still holds its `Prepared` and the goal it composed — which is the
  whole point of the ruling, which is about a typed goal that was lost.
- **An unanswerable table refuses nothing.** brazen unable to answer (a config
  it cannot parse) is an empty table, never an error — the `Providers` read's
  own "asked, never stored" contract — and the fire passes to the gates behind.
...
role naming an unsigned keyed row beside a signed one is readied by the signed
one") being exactly the defect that was then reproduced live — but a row that
holds a credential and a model id that row does not serve is still a fire that
fails at the first dispatch, through §8.3's auth-shaped step failure and the
`failure` clause bl-d774 put on the row, against a wall that is a fact by then.
That is the right side to be wrong on: whether a row serves an id is a question
only the provider can answer and only over the network, and the ruling's "must
not add a round trip" forbids asking it here. The same defect one rung over — a
bare start into an unsigned wall — is the same read and the same answer,
because there is only one `Prompt` door and one wall to judge.
...
| Stop | `litany stop <ws> <agent>` — no toggle: litany's stop takes the whole subtree since its bl-3114, so `--stop-children` named a choice it no longer offers and yog stopped passing it (bl-6efc). The boundary's `children` field is accepted and ignored until the next `PROTOCOL` bump; `stop_children` on the conversation reads still rides, naming the *consequence* — whether this stop reaches further than the conversation in hand | short, piped |
...
| Pin a live conversation's **workflow** to a named lineage's head, or let its followed lineage govern the workflow again (§9.4 as amended by bl-b680; litany ARCH §6, upstream bl-5c02) | `litany workflow <ws> <agent> --config <name>` \| `--clear` (ws) | short, piped. It writes (or deletes) the **standing** `refs/litany/workflow/<agent>` at the lineage's head and touches no branch: only `workflow.yaml` moves, the rest of the config keeps following the tip, and the mark names a commit so it is a pin as well as a switch. Nearest mark on the descent wins at the next step boundary. `--config` IS named here, unlike `retarget`'s — choosing among lineages is the act's whole point, and `request/lineages` is the browse that says what there is to name. Every refusal is litany's and precedes the write (`Action::Workflow`, REMOTE §9.24). The read is not a verb: it rides `reply/governing` as `workflow_mark`, derived by yog over the same refs (`config_edit::branch::workflow_mark`) |
...
| Login provider | `bz --login --provider <row>`, plus `--browser` on a row serving no device flow (`Action::Login`; the remedy an auth-failed step names) | streamed-piped (§8.3); same physical retarget. The row is offered only when brazen's table says `auth = "oauth2"` (§8.3 as amended, bl-b4e5), and the flag follows that same table's `device` column — one read, at the spawn (§8.3 rule 1 as amended by bl-61bf, landed bl-7c9f). The spawn's locus is the engine's, as a boundary act (REMOTE §8.3, landed bl-c285) |
...
- **brazen `[ingress]`/`--serve`, credentials editing** — out of scope;
  ...
  **The flow-selection rule and the capability source (bl-b4e5), three
  ...
  1. **The flow is the row's own capability (amended bl-61bf): the device
     ...
     *required* fields of every `oauth` block while the `device` block is
     optional, so `--browser` remains the floor every oauth row can serve; a
     ...
     read off brazen's own table (rule 2). **Landed at bl-7c9f**: the branch is
     `login::Flow`, chosen ONCE in `login::runs::Runs::start` off the same wall
     lens the spawn folds through, and the `--browser` word is dropped exactly
     where `ProviderRow::headless_login` says the row declares a device
     endpoint. The run-by-hand fallback is the argv that was attempted in the
     flow it was attempted in (rule 3), so it stays a command that would
     actually succeed.
  2. **Loginability is brazen's `auth` column, never a yog-side
     ...
     see rule 4. **The projection carries the device-endpoint fact too**
     (bl-7c9f, over the brazen release that answered the bl-c5fe ask): the
     listing's `device` column names the headless flow a row serves in brazen's
     own `device.style` spelling, and rule 1 as amended (bl-61bf) makes that
     the flow key. A style rather than a boolean, because the endpoint's
     grammar is the row's fact too — brazen selects the wire from it — while
     yog's one reader asks only whether there is one. A row naming none, and a
     listing written before the column existed, both read as browser-only: an
     unanswered question grants no capability, exactly as it refuses none.
...
refuses and ambiguous refuses, in the resolver's own words.

**The window that used to stay is closed, and the disk rung is where it closed**
(bl-802a). Between the fire answering and its detached driver committing
`agents/<id>` there is no branch anywhere for any rung to find, and this section
used to conclude that the handle therefore refuses — honestly, naming the token.
It is not honest: it is the sentence a name that never existed earns, arriving
for a name the engine itself minted one second ago, so `start` then `follow` —
the first pair every operator types, and the whole of a two-seat handoff —
failed every time it was typed (measured 2/2, self-healing in one to two
seconds). **"Unknown" was never instantly knowable.** A name is minted, then
materialized; a read that lands inside that gap has not learned anything yet.
So the third rung **holds** before it concludes: it looks again six times, half
a second apart, and only then refuses (`boundary::address::agent::settle`).

The alternative this section named — *waiting on the driver inside the fire* —
stays refused on §8.1's own terms, and it was also the weaker fix: it makes the
property true for a client holding the receipt and for nobody else. Held at the
resolution, every client gets it — the seat that was handed the name, a script
that types it, the second seat told to go and look. The cost is paid by the case
that cannot yet be sure and by no other: a name that resolves pays one look,
because rungs one and two answer first and this one answers on its first try;
only a name with **no** answer waits, and a genuinely wrong name therefore
refuses in three seconds instead of instantly. **The §3.3 ladder's
...
- **Queries** are I1 derivations: everything that populates. Their carrier is
  ...
  notice**, the wound the conversation stopped on, said in the transcript where
  the operator is reading rather than in a steps query nobody opens on a
  conversation that looks stopped. Both folds are one question — *what is this
  conversation doing that its `messages/` cannot say* — and they are exclusive
  by derivation, a wound being claimed only of a step nobody is driving, so the
  answerer matches on the live tail alone and a healthy in-flight conversation
  pays for no steps walk. The wound crosses the wire in its own vocabulary,
  spelled by the one encoder that owns it (`steps_view::wire`), never as the
  sentence built from it — the `Compacted` counters' rule, read on a third
  shape. The *pin* is not answered at all, and
...
consumes it. I7 holds: a deposit *is* the explicit user action; the consumer
adds no spontaneous mutation.

**A claim is held under an OS file lock, and a lost reply is answered in
doubt — never re-run** (bl-d1f1). The claimant locks the deposit *before* the
...
writes the reply slot an **in-doubt refusal** naming the gesture, so the
depositor's poll terminates with a sentence instead of waiting forever, and an
ops row (`gesture-debris`) records it. The refusal's instruction is the whole
recovery contract: *read the world before acting again — a gesture is not
idempotent (REMOTE §9.8), so a re-send is a second act, not a retry.* The
sweep never re-runs the gesture: redelivery is the mailbox's trade (REMOTE
§5.3), taken there because the far end holds a stable id to dedupe on; here
"claimed, never ran" and "ran, reply lost" are on-disk identical, so in-doubt
is the only honest terminal answer, and exactly-once is not yog's to promise —
the effect belongs to a subprocess (`bl`, litany), and only idempotency there
could make a retry safe. A re-deposit under a spent id refuses (`deposit`
checks the claimed file beside the inbox, and minted ids are never reused), so
the path this paragraph once instructed — "re-deposit to re-run" — is refused
mechanically where it used to be advice.
...
**five since bl-23bd**, the §9.4 tuning pair joining them:
...
- **`PickModel { workspace, role, provider, model }`** — §9.4's pointer. It read
  *"§9.2 and §9.3 composed by one gesture"* until bl-23bd corrected it here: that
  composition died with litany's cross-check (bl-d9cb, §9.4 *"One gesture, ONE
  file"*), and the reason it cannot decompose into the two above is now simply
  that it is one write with its own gates. Line: `/model <role> <provider>
  <model-id>`.
- **`Tune(model_pick::Tuning)`** — §9.4's **tuning pair** (bl-23bd): the same
  role assignment's two optional fields, `effort: low|medium|high` and
  `priority: true` (litany ARCH §4.3, upstream bl-acba/bl-f587, consumed at the
  0.0.6 pin). Lines: `/effort <role> <low|medium|high|off>` and `/priority
  <role> <on|off>`.

  **Two ops, one carrier, and both halves of that are load-bearing.** Two *ops*
  rather than a widened `/model`, because REMOTE §3 makes a new op free while a
  changed shape is a version bump — and because a toggle that made the operator
  restate provider and model would force two assertions they did not come to
  make. One *carrier*, because every layer beneath reads the two as a pair (one
  line reader, one codec file, one executor, one help section, one file
  written), which is §12's fold criterion exactly; the fold is in the carrier
  and never in the surface, so each still spells as its own verb and its own
  envelope `op`.

  **`off` removes the line.** litany reads both fields as `Option`s and states
  that `false` and omitted are one fact, so writing `priority: false` would be a
  second spelling of an absence the engine already spells once — and an operator
  would read it as a third state. Removing is also what makes the gesture
  idempotent, which is what lets a seat send it without first reading the file.
...
already existed verbatim as `yog bz --login` (§16.7 W10), so a variant would be
a second spelling of a gesture that already had one. That argument held only
while the wall was *this box's*: `yog bz --login` signs in the wall of the
process that runs it, and once a workspace can be held from another box (REMOTE
§8.2) the seat's shell is the wrong box entirely — the credential landed in the
seat's wall and the host's stayed empty. So the sign-in is now `Action::Login`
plus the follow-class `Query::LoginTail`, executed on the ENGINE inside the
NAMED workspace's wall (REMOTE §8.3). `yog bz --login` is not retired by it and
is not a second spelling of it: it is the same act reached from a shell already
standing in the world, which is what §16.7 W10 advertises. The variant is the
...
no write to share a verb with — the §8.3 sign-in became a variant of its own at
bl-c285, and it is a different verb, not this read's write — so
...
- **`Query::Attention` → `Reply::Attention(Vec<QueueRow>)`** (`/attention`,
  ...
  gestures take), its display name, its state, which signals fire **and what
  they say** (`says` — the firing rules in words, from `AttentionKind::says`,
  the one home for that sentence, so a seat that announces one announces this
  engine's wording; bl-09ef), what it last said, its age and its pending-mail
  count. Per **agent**, not per conversation
...
- **`Action::MarkSeen` → `Reply::Acknowledged`** (`/seen`, `{"op":"seen",…}`)
  ...
  the address it acted on beside it** (bl-5cfe). It answered with the remainder
  alone until then, and that receipt could not be told from a plain
  `/attention` read: the row the act touched is *precisely* the row the
  remainder no longer holds, so the one fact the answer existed to report was
  the one fact it did not carry, and the workspace's attention count went down
  with nothing saying why. `Reply::Answered` refuses the same lie one verb over
  ("a receipt that lied"), and `/seen` is the precedent that doc cites. The
  help line was the same defect in words — *"answer the selected conversation's
  place in the attention queue"* reads as a scoped `/attention`, beside a list
...
second spelling of a gesture that already exists. (This used to cite §8.3's
login as the precedent; bl-61bf overturned *that* case for a reason particular
to it — the act had to change boxes — and the rule it was cited for is
unaffected.)
...
- **The control is a world-tools shim** (§16.4's re-exec pattern, seated
  ...
  **absolute path** in the authored block — never PATH-resolved, so no host
  ...
  command line classified by that same ruleset, an input naming a `url` read
  as open-world (bl-c6f0), anything else `opaque`, which
  ...
  **A name with nothing to read needs a way out, and it is a row** (bl-b65d).
  The lane above reads a command line; a routed tool that carries none holds on
  every call, and the class that carries none by construction is the MCP one —
  a pinned server's tool reaches this control host-qualified with an input its
  own schema shaped (`box2_fetch {"url": …}`, thrall DESIGN §6; REMOTE §5.4
  rules the bridge is thrall's and the wire gains nothing), so no line will ever
  appear. Held forever is a park answered by reflex, and the only standing
  answer before this was `table:` / `opaque: pass`, which reopens the whole
  class bl-72bd closed. So `capability.yaml`'s existing `rules:` grammar admits
  a routed tool's **whole host-qualified name** as its program — `rules:` /
  `  box2_fetch: open-world` — and `classify/routed` consults the operator's
  rows with the request name as its one word before answering opaque. No new
  key, no new file, no new class. Four properties are the whole design. The
  class is the **operator's own statement** of what that tool on that box
  reaches, informed by what `thrall mcp pin` printed of the server's
  annotations and by nothing the wire carries — an advertisement states no
  effect (REMOTE §5.1) and yog infers none from one. It is **host-qualified**
  because the same server on two boxes is two trust decisions (REMOTE §5:
  locality rides in the name). A name row **never outranks a line**: it is asked
  only where the input carries none, so no row on `box2_shell` softens a
  `find … -delete` the line itself classifies destructive. And **only the
  operator's rows answer a name** (`Policy::stated`) — a shipped row states what
  a *program on a command line* reaches, so handing `rm` the tool the `rm` the
  program's row would be this control inferring a class for an invocation it
  cannot read, which is the guess bl-72bd deleted. It stays a class adjudicated
  per invocation, never a name allowed: bl-7fc8 stands.

  **The cost was measured, and it inverted the incentive.** A round-2 drive over
  two feet — one advertising a shell-shaped tool, one advertising three narrow,
  argument-checked, path-confined admin tools and no shell at all — spent **11
  holds and 11 operator releases on one goal**, every one of them on the foot
  with no shell, while every shell command line on the other passed unattended
  because a line is readable. The narrower tool document was the punished one,
  and an operator who notices fixes it by adding a shell — which is the gate
  arguing against its own purpose. The row removes that, and it removes it **per
  tool rather than per call**: six calls of one log reader with six different
  paths were six parks, and one row is all six. A once-answer still scopes to a
  `tool_use` id, as it must; what changes is that a tool the operator has
  spoken about never reaches the hold again.

  **And the input's other operand is read like the first** (bl-c6f0). The row
  above is the way out for a tool this control can say nothing about, and a
  fetch tool is not that tool: its input names a `url` (or `urls`), and
  reaching an address off this machine is the *definition* of the open-world
  class — network egress — so `classify/routed` answers open-world rather than
  parking it. `thrall mcp pin`'s own fetch entry is the shape
  (`{"url": …, "max_length": …, "raw": …}`, thrall `docs/tools.example.json`),
  and the phone foot's `http` tool is the same one, so without this every fetch
  on every box was an operator keystroke or a row.

  It is the command line's reading applied to the other operand a routed schema
  carries, and it revives nothing bl-72bd deleted: an operand is what the
  invocation POINTS AT, never an assertion it makes about its own effect —
  which is the difference between this and the envelope's `invocations:` list
  below. Four bounds. It is asked **after** the command line and **after** the
  operator's row, so neither is softened. It answers only for an input that
  actually names an address — a blank, an empty list or a non-string names
  none, and the hold stands. It can only ever answer open-world, so no shape of
  input reaches `read` by this path. And the class is still adjudicated per
  invocation: the shipped table passes it, a `table:` / `  open-world: hold`
  workspace parks it, and a raised floor holds it like every other class above
  read.

  **What it does not solve**, stated rather than papered over: a tool whose
  input names a url and whose act is wider than fetching it — a delete keyed on
  a resource address — reads open-world and passes, because an operand names
  the reach and not the verb. The answer is the row, which outranks this
  reading; there is no second heuristic here, and adding one would be this
  control guessing at a verb it cannot see.
  ...
  **The verdict goes to the party that can act on it, and on this leg that is
  ...
  is sitting at it and an in-band decline costs no park. On a foot it is not an
  answer: the drive that filed this had `rm -f <dir>/*` refused destructive,
  and the model probed the confinement over three steps and ran
  `cd <dir> && rm -f -- *` — 115 MB gone from another machine, with no hold, no
  attention item and nothing the operator could have answered. So on the routed
  leg the table's **refuse becomes a hold** (`Ruling::for_the_operator`, read in
  `Answers::ruling` off `classify::Leg::of` and therefore off the same intrinsic
  map the classifier folds names with). It also opens the door that was shut:
  under a refusal `answer` has nothing to release, so the one destructive act a
  server exists to make safe — freeing disk — could not be authorised through
  the boundary at all. One direction only: a hold is softened into a refusal
  nowhere, and the once-answer stands **ahead** of the leg, because an operator
  who answered this exact `tool_use` id has made the decision and re-parking it
  would ask them what they just answered.

  **And the writable root is not on the other machine.** The evasion was not
  cleverness; it was an operand test that means nothing on a foot. A relative
  operand resolves against the agent's own cwd — the engine's worktree, inside
  the root — so a bare `*` after a `cd` read as a target write while the
  absolute spelling of the same act read as loss. The special case was the root:
  **this control vouches for no path on a machine it cannot see**, so
  `classify/routed` judges every command line against a root with an **empty**
  writable set (`far`), every `ByRoot` row takes its `outside` class, and the
  `cd` chain classifies exactly as the direct form. No modelling of `cd`, no
  glob expansion, no new arm — one invariant instead of an answer per spelling.
  What it costs is that ordinary loss-shaped work on a foot now parks; what it
  buys is that the park is answerable, which the refusal never was.

  **The hold sentence names the row.** The opaque clause the mark carries spells
  the way out with the tool's actual name and the class words the file accepts
  (`box2_fetch: <class>`, one of read / target-write / process / open-world /
  destructive / secret — `Effect::reach_words`, the same list `Effect::of` reads
  back, so the offer cannot drift from what parses) and says that a key ending
  in `_` states it for a whole box. That second form is bl-1772's: measured, the
  per-name row alone inverted the incentive — a box advertising three narrow,
  argument-checked tools drew a park on every call while a box advertising one
  raw shell ran unattended — so `beta2_: read` vouches for everything that
  client advertises, and the separator is what makes it unambiguous, no
  advertised name ending in one. A park whose remedy the
  operator must go and find is answered by reflex; this is bl-68e1's
  `NOT_A_REMEDY` discipline from the other direction.

  Two consequences worth stating rather than discovering. The **compactor's
  procedure pair, `python`, `search_history`, `remember`, `read_tool_output`
  and yog's own `clients`** now
...
- **Standing policy is one file, and it is an override** (bl-765d):
  ...
  since bl-b65d the same rows are what a **routed tool name** is classified by,
  the program being the whole host-qualified name and the operator's rows the
  only ones asked) and
...
- **And it derives the CLASS the wide scopes stand over** (bl-94a5). The
  gesture carries `(workspace, agent, answer)` where the answer is a verdict
  **and** a scope (`call` — the default — `conversation`, `workspace`), and the
  class is derived exactly as the id is: read back out of the sentence yog
  itself wrote into the mark (`control::reason::class_of`, the inverse of the
  `reason` that wrote it — the two are one file with one test holding them
  equal). So a seat spells a decision, never a key. Two refusals, both
  fail-closed and both *before* any row is written: a mark whose sentence names
  no class takes `--scope call` only, because a key nobody can compute is a
  grant nobody can bound; and a destructive or secret call takes `--scope call`
  only by ruling, the refusal naming the `capability.yaml` `rules:` row that
  makes it standing where policy is read and deleted. The key is
  [`judge::class_key`] — the tool AND its reach, never the tool alone, so a
  `Bash` released for a read carries no later destructive line.
- **The floor is the same fold written the other way** (bl-94b4, VISION §4.9's
  ...
  `/answer pass`. It also **suspends every standing answer** (bl-94a5): a
  conversation- or workspace-scoped grant is the auto-approval a revoke exists
  to take away, so under a floor only the call in front of the operator and the
  workspace's table are consulted. That is not a second rule — it is what makes
  a standing answer revocable at all, since the floor is what parks the next
  call of a released class so it can be answered again at the same scope. It matches by descent prefix, so it stands over the named
...
this section already gives, restated as §9.5's first justified fallback — which
since bl-dc3f is said in the answer rather than only here: `Query::ReadConfig`
carries an **empty** `settings` for this destination, because yog declares no
TOML reader and `bz` is the only lawful parser.
...
**§9.5 amendment, and its reversal (bl-dc3f, bl-9c8a): both files are raw
text.** `models.yaml` had a schema while yog read one fact out of it — the
`context_window` block, §5.1 #35's denominator, answered as typed rows — and
that fact moved to the step record brazen writes it on (§5.1 #35), so yog now
reads nothing out of this file at all and `schema_for` answers it nothing, the
same general path with an empty schema `workflows/*.yaml` always took (litany's
workflow DSL is litany's, and yog holds no grammar for it). **Both answer
through the same carrier**: `Query::ReadConfig` returns the raw `text` and a
`settings` array beside it — empty for both of these, so the difference between
a typed destination and a fallback is a value in one answer rather than two
shapes a seat must tell apart.
...
**Nothing migrated then, because reading was never the problem** — the grammar
was anchored line reads (§9.4), so an operator's existing four-field entry kept
parsing and `context_windows` still keyed on `model_id`; yog wrote less and read
the same. Nothing migrates now either, for the opposite reason: since bl-9c8a
yog reads nothing out of the block, so a file hand-authored to any of its shapes
is inert to yog and litany alike, and gets no warning.
...
**`models.yaml` has no reader in yog since bl-9c8a.** lernie 0.0.10 retired
the `models:` table (litany's bl-35e2): the file's whole load shape there is one
optional `adapter:` field, and a leftover `models:` block *"is ignored on
parse"*. Between bl-d9cb and bl-9c8a the block lived on as **yog's own table**,
one fact wide — `context_window`, the §5.1 #35 denominator — written by a
Declare control, edited by §9.5 typed rows and adopted onto the snapshot on the
15 s sweep, and bl-d9cb left open whether a number only yog read belonged in a
litany file at all. The answer was neither home: the window is the provider's
fact, brazen states it in band on every `Usage` event, litany's own compaction
trigger divides by that in-band number, and a declaration only yog could see was
a second representation of it (§5.1 #35 carries the ruling). The table, the
Declare control, the typed rows, the snapshot field and the sweep arm are gone;
this destination is the raw-text editor litany's own posture always asked for.
...
(§9.5) — since bl-dc3f **actually answered**, in the `settings` array beside
that read's own `text`, with each role's `provider` carrying the live-table
judgement `grammar::is_unknown_row` makes and its `tools` carrying the list
control; every other path in a config commit — `souls/**`, `descriptions/**`,
`workflow.yaml`, `manifest.yaml`, `version` — is prose or litany's own schema
and is text with an empty `settings` (§9.5's third justified fallback). Before this a config commit could
...
- **Nothing offered is unroutable.** Every listed candidate is a model brazen
  ...
  **Row existence is not request-shape compatibility, and this claim used to
  ...
  rejects every request with nonempty `tools`. A yog turn has nonempty tools —
  the role's own grant, plus whatever litany splices in (the host injection and
  the calling role's procedure) — so `/model worker claude-code <id>` advanced
  both config halves and then failed the next worker start before any network
  call, on *"claude_code carries no tool declarations; use the `anthropic` row
  for tools"*.

  **The reason this used to read "unconditionally" is gone, and the refusal is
  not** (bl-52b7). `tool_host::Injection::tools` once declared `clients` for
  every agent of every role, so *every* turn had tools whatever the config
  said; since ruling 3 the injection is inside the role's grant (REMOTE §5.2),
  so a role granted nothing is now genuinely tool-less. The pick still refuses
  a row that cannot carry tools, because the judgement is about the **row**,
  not about what one role happens to be granted this week: a workspace's roles
  are edited, a config lineage is forked, and a pick that was lawful only for
  an empty grant would fault the first time one was widened. What changed is
...
**The assignment is wider than the pointer since bl-23bd** — it may also carry
`effort:` and `priority:`, the §9.4 tuning pair — but that widens the *entry*,
not the binding: the two knobs shape how a call is made, and neither takes part
in deciding which model answers it. That is why they are a sibling gesture with
no provider gate rather than more fields on the pick.
...
**What became of `models.yaml`.** The file is still litany's — it carries
`adapter:` — and the `models:` block inside it was, from bl-d9cb to bl-9c8a,
**yog's own table with exactly one consumer**: `context_window`, the §5.1 #35
fullness denominator. bl-d9cb deleted the picker's seeding write and kept the
fact at that one home, under `DEFAULT_CONTEXT_WINDOW` and a comment naming the
figure it fed; it also named the served window as *"a query, not a field"* and
sketched a read-time query over brazen's model cache should the figure ever
prefer one. **bl-9c8a took that to its end and the home went with it:** brazen
now performs the cache read itself on every call and stamps the resolved row's
window on the `Usage` event the engine records per step, so the query was
already answered in the general path, and a declaration only yog could see was
a second representation of a fact litany's compaction divides by (§5.1 #35). No
gesture declares a window and no file carries one; the seat for a row that
serves none is brazen's provider row, upstream. bl-848f's distinction — a
fabricated default sitting indistinguishably beside a served number — has no
instance because there is no default.
...
nothing more — the `context_window` beside it reaches yog on the usage event,
not off this list (§5.1 #35).
...
**The grammar now creates a field line as well as moving one** (bl-23bd), and
the terms are narrow. `set_field` replaces a four-space line that is already
there, and that refusal was right for every caller it had: they *move* a value
the file states, so an absent line means the file is not the shape they were
told it was. An **optional** assignment field inverts that — absent is its off
state, so its first write is always an insert — hence `upsert_field`, which adds
the line last among the fields the entry already has, and `remove_field`, which
takes it away. Neither widens what the grammar will *recognize*: the block form
is unchanged, the entry must already exist (creating a role would be the YAML
transform this never is), an inline block key still refuses, and every other
byte — comments, sibling fields, sibling entries, the blank lines between them —
is preserved exactly as `set_field` preserves it. The alternative was
`set_entry`, and it is the wrong tool for a `roles:` entry by its own doc: it
replaces the whole entry, so an unstated `tools:` line or a comment would not
survive.
...
**The workflow mark: one control fact with a per-agent override (bl-b680;
litany ARCH §6, upstream bl-5c02).** Follow-the-tip has exactly one carve-out
in litany, and it is the workflow: `litany workflow <ws> <agent> --config
<lineage>` writes the **standing** ref `refs/litany/workflow/<agent>` at that
lineage's head, and from the agent's next step boundary on *that* commit's
`workflow.yaml` governs instead of the followed tip's — nearest mark on the
descent wins, so marking a root switches its whole tree and a child's own mark
overrides it; `--clear` deletes the ref and the tip governs again. It is not a
retarget with a name: a retarget moves the whole config by re-forking the
branch, this moves one file's provenance by writing one ref and touches no
branch, and the mark names a *commit*, so it is a pin as well as a switch —
`providers.yaml`, the souls and the manifest keep following the tip while it
stands. yog's half is two things and a deliberate absence. The gesture is
`Action::Workflow { workspace, agent, config: Option }` (§8.5, §8.2's table):
`/workflow <lineage>` and `/clear-workflow`, two ops on the wire on the pin's
precedent, the workspace-bound `litany workflow` spawned piped so every refusal
— an unknown lineage, a head whose `version` or `workflow.yaml` will not parse
— comes back in litany's words, each of them ahead of the write. The **read is
a field, not a verb**: `Query::Governing`'s answer carries `workflow_mark`
beside the followed commit, derived by `config_edit::branch::workflow_mark`
over the same refs litany's `nearest_mark` walks — holder, marked commit, and
the `config/*` name standing on it when one still does — never by reading the
verb's one-line stdout back (bl-b95e: content is diagnosis, not a trigger).
And the absence: there is **no read enumerating what a mark may name**,
because `Read::Lineages` already is one — a mark names a lineage's head, and
the harness `workflows/` pool the config editors reach by name is litany's
template pool, which no mark can name. REMOTE §9.24 carries the wire's half.
...
**The carrier is `Query::ReadConfig`'s own answer (bl-dc3f).** It answers a
[`ConfigView`] — the destination's raw `text`, and beside it `settings`: **this
file's schema applied to that very text**, one flat entry per declared setting,
each carrying `entry`, `name`, the `control` it is edited through *with the
bounds that control judges by*, its `help`, the `value` the file currently
spells, and — absent when there is none — the `fault` saying why that value
cannot be used as it stands. One answer, both views, and the file stays the
single fact: the two halves are read from the same bytes in the same call, so
they cannot be of different states.

**One answer rather than a second query.** A `Query::ConfigSchema` was the
alternative and is refused: it would answer a *description* that a seat then had
to join against a separate `ReadConfig`, which is two reads of one file with
their own staleness between them — and the schema alone cannot say what the
values ARE, so the join is not optional. The settings ride the read that already
returns the bytes they are read from.

**The bounds ride the control** because §9.5's verb is *judge at input*: a seat
that knows a field is a number but not its range cannot do it, and yog's ranges
are `src/app/cadence.rs`'s own consts, so a seat re-deriving them would be a
second authority on the clock's limits. The provider `fault` is the same
`grammar::is_unknown_row` §9.4's pick gate calls, answered rather than
re-implemented — which is what makes *"they cannot disagree"* true across the
boundary and not only inside this process.

**A file with no schema answers an empty `settings`, never an absent field.**
The three fallbacks below are §9.5's own "general path with empty input, not a
branch", and a seat holding no settings is already showing the raw editor.
brazen is asked for its provider table **only where a schema declares a provider
control**, so reading `cadence.yaml` spawns nothing, and the table is read
inside the wall the gesture names (bl-fcd5) exactly as every other §9 read.

**The write half is unchanged and stays whole-text.** `ApplyConfig` carries the
file's whole text through the unchanged pipeline, so a typed edit is a seat
composing that text and applying it — the same act the raw editor makes.
`config_edit::form::write` (the anchored single-line rewrite, and the one home
of how a typed setting is spelled back) therefore still has no production
caller; it is kept rather than deleted for the reason the §11 sweep gives
generally — *keep the one derivation, delete nothing that is a single home* —
and the residue is bl-7cc8's class.
...
| `models.yaml` (§9.2) | litany's `adapter:` and whatever else the operator keeps there | **raw text** — fallback since bl-9c8a. It had typed rows over yog's own `models:` block (`model_id`, a bounded `context_window`, and a *declare a new model id* control → `declare_model`), and before bl-3ffa two more (`provider`, `capabilities`) with no reader anywhere. All of it is gone with the one fact it edited, which now rides the step record (§5.1 #35): a control over a fact nothing consumes is a setting that cannot matter, and no control KIND died with them — the number is `cadence.yaml`'s, the text `providers.yaml`'s |
...
All three are the **same answer**, `Query::ReadConfig`'s raw `text` with an
**empty** `settings` beside it (bl-dc3f) — so a fallback is legible as itself
rather than as a typed read that found nothing: a fallback exists to hand over
text no schema can model, so it must be the whole text and nothing about how it
is presented may hide it. bl-2622 is the record of that
...
### 9.6 The learning loop's operator half (bl-dd88)

litany's reviewer stages a real config patch on `proposal/<reviewer-id>` — its
own `docs/DESIGN_LEARNING_LOOP.md` §3 — and the whole design turns on **a
person being able to see what was learned and veto it**. Adopting the loop was
already two gestures a seat could make (`/config branch default workflow.yaml`
and `/model reviewer …`); reading, accepting and rejecting were `litany
proposal` and nothing else. So from any seat — the window, the phone, `yog
gesture` — a staged patch was invisible: `/lineages` does not enumerate
`proposal/*` refs and `/governing` reads the followed commit, never a
candidate. On a server install (§10.1) that made the veto an `ssh` and a
container `exec`, which is exactly what the §8.5 boundary exists to make
unnecessary.

**Two gestures and no third.** `/proposals [<id>]` is the read — the listing,
and, when an id is named, that proposal **whole** (message and diff). One verb
at two depths, the shape `/files` and `/work-diff` already take: a seat that
names an id has already been answered the row it names, so a second query would
be a second derivation of one subject. `/proposal <id> accept|reject` is the
settle, and **both words are required** — a settle that took "the only one"
would do something different the day a second proposal was staged, and a
verdict that defaulted would make the destructive half the easy one.

**Both are members of the §9 config family, because a proposal is a candidate
config commit.** The read joins `Read`'s five, the settle joins `Write`'s four
(§12: the write family folded onto one carrier for this, matching its questions'
own since bl-719a). Nothing about the fold moves a wire spelling — each member
keeps its `op`, its slash verb and its help page — which is what let the corpus
regenerate byte-identical over it.

**The read is yog's and the act is litany's, and the split is not arbitrary.**
yog already reads this workspace's other two ref namespaces itself — `agents/*`
for the §7.1 tree, `config/*` for the §9.3 browse — so a listing over the third
is that same derivation through the same scrubbed doorway, and it answers typed
rows rather than a rendered table a seat would have to parse back. The settle is
not read arithmetic: accepting is a **compare-and-swap fast-forward** whose
expected old value is the freshness the listing showed, and re-implementing it
here would be a second home for a rule whose failure mode is a lost race. So the
settle is a workspace-bound `litany proposal`, piped like every other §8.2 verb
and leaving that family's §4.2 row — which is also how a stale proposal, an
ambiguous one and an unknown id reach the operator in litany's own words, naming
the tip, the lineages and the pool respectively.

**Freshness is derived at read time and never stored.** A row's `fresh` is its
parent compared against the config heads in the same pass, and the accept
re-derives the same test as git's own `update-ref <head> <new> <old>` — so a
proposal cannot be listed fresh and then accepted on a field that has gone
stale, and a race is refused by git rather than by a check that could lose to
it. The remedy for a stale proposal is a reject and the next checkpoint, never a
three-way merge of somebody's memory: its reviewer read a config that no longer
governs.
...
**Six facts had no boundary spelling, and each was filed rather than fixed
here** — the sweep's real product, since a doc pass may not add a surface. Each
row below carries its ball, and a struck row is one that has since been
answered:

| Fact | Ball |
|---|---|
| ~~the §6 desktop escalation: `src/alert/` derives an announcement no caller spends and no reply carries~~ — closed (bl-09ef): **deleted**, not wired. The announcing is a seat's — a notification belongs on the box the operator is looking at — so what crosses is the wording (`AttentionKind::says`, on every queue row's `says` key) rather than the act | bl-09ef |
| seat-shaped derivations still in this crate with no carrier and no caller — drafts, the §3.3 name preview, the fork composer, `science::{compose,respdiff}`, the §8.1 start gate | bl-7cc8, **landed**: deleted. The §8.2 `enabled` predicates were the sweep's one dead premise — three of them DO cross, as `nudgeable`/`stoppable`/`stop_children` on the §8.5 agent answer — so only the one with no carrier (the message gate's composer-text half) went with them. **The rest of the same fact closed as bl-33e9**: `nav::menu`'s §11 context-menu roster, the actions root's form rules (`work_dir_refusal`, `goal_present`, `new_prompt_enabled`, `create_ball_enabled`, `new_ball_hints`) and the §3.5 `close_enabled`/`unclaim_enabled`/`assign_enabled` the roster was the last reader of. None wanted a carrier: a gate over a **typed input** is judged where it is typed and refused again by the executor at the fire (the spawn boundary names a bad work directory, `bl` refuses a claim of a bound ball; and since **bl-54c1** the `Prompt` door refuses a blank goal — §8.1 step 2's invariant restored at the fire rather than as a form rule, its one home being the door every spelling passes), and a gate over a `JoinState` is a one-line fold over the `BoundBall::state` `Query::WorkspaceBalls` already answers — which REMOTE §9.4 leaves to the seat by its own rule, *a gate that is not derivable from a row goes on the row*. What `actions` still holds is the three that cross |
| the derived cadence periods a seat must honour, `wound_grace` first among them | bl-776a, **landed**: none of them crosses, because none has to — `late_pass` and `stale_after` were already spent on this side, and the wound grace is spent here now too (§7.3) |
...
its corpus, but no cell may name one. That exemption holds only while the
corpus is actually reached from its production module, so the precondition
this table leans on is checked in `tests/module_reachability.rs`: every `.rs`
under `src/` is declared by a `mod` in its parent module (bl-862e — a corpus
nobody declares is dead text this table skips by rule, `rustc` never opens,
and coverage reads as clean because nothing in it ran).
...
|---|---|
| `src/actions/mod.rs` | the action root (ARCH §3.4/§3.5): the §8.2 enablement predicates **that cross**, and since bl-33e9 there are no others — `nudge_enabled`, `stop_enabled` and `stop_children_offered` are the §8.5 agent answer's `nudgeable`/`stoppable`/`stop_children`, so a seat is told rather than asked to re-derive. Pure, and the verbs themselves live in `verbs` (§8.2). The `enabled` submodule those three sat in folded back in when its seam went: it was split from the composer's own rules at §12's budget, and the composer's own rules are the half that left. What left, in two waves: the message gate's content test (bl-7cc8), a composer's text, so this crate states only `present`; then the form rules — `work_dir_refusal`, `goal_present`, `new_prompt_enabled`, `create_ball_enabled`, `new_ball_hints` — and the §3.5 `close_enabled`/`unclaim_enabled`/`assign_enabled` (bl-33e9), the first family judging a seat's typed input that the executor refuses again at the fire, the second a fold over a `JoinState` every `Query::WorkspaceBalls` row already carries (REMOTE §9.4: a gate that is *not* derivable from a row goes on the row). **The seat's own RAM left with the face** (bl-7942, deleted bl-7cc8): the composer drafts keyed by target (§5.3), the `ActionsState` that held them beside the selection and the birth-config box, and the clean-send predicate that cleared one |
| `src/actions/verbs{,/balls,/balls/verb,/balls/edit,/bound,/workflow}.rs` | the §8.2 verb dispatchers + opslog wiring, cut on the table's own seam when the V2 attempt joined it (bl-dc0c): `verbs` the litany family — message, the attempt's `dispatch`, stop, scan, and the §9.4 `retarget` change-of-lineage (bl-2d19, re-scoped bl-e654) — acting on a conversation in a workspace; `workflow` the §9.4 workflow mark's verb (bl-b680), its own file beside the table because the table sat in the pre-split band: `--config <name>` sets, `--clear` clears, and the bare read litany also answers is never spawned, because that answer rides `reply/governing` derived rather than scraped; `balls` the `bl` family, acting on a ball in a project and stamped `--as` its §3.2 claimant to a verb; `balls/verb` **the family's carrier** (bl-92d3) — the one `Verb` the §8.5 roster holds a single row of, split off `balls` at the pre-split band on the seam the family already has: the parent spends an argv, this is the intent the boundary carries to it, and it is where `Action::Ball`'s address answer lives so the project table reads one arm instead of five; `balls/edit` **what a create/update carries** (bl-dbde), split at the budget on the seam `balls`' own doc draws: the verbs act on a ball in a project, this is what a ball is made of, and only the second grows every time balls learns a field — the boundary carries these types WHOLE, so the roster, the codec and the executor read one vocabulary rather than three copies bridged by an `of` constructor; `bound` the workspace-bound spawn seam every litany verb takes (§8.2's workspace-bound rider, bl-bf79) — one fold laying the wall and the name, so the family owes no per-verb decision |
...
| `src/app/balls.rs` | the engine's read of the live `bl` projection — the ops tail and the two post-verb dirty marks (§5.1, §7.2); the §3.4/§8.1 start hand-off. **The §3.2/§3.5 read half is gone** (REMOTE §9.7, bl-b4b5): `ws_balls`/`roster_ball_rows`/`bound_ball` and the whole `spend.rs` beside them folded the window's own join and bills on the paint thread; one workspace-addressed `Query::WorkspaceBalls` answers the listing *with* each ball's figure, and the roster's partition and the ▶ Continue row's object are pure selections out of it (`nav::balls`). `targets.rs` emptied out when the ball Move was retired (bl-6c28). The §7.3 standing-failure query per origin — `last_failure`, and `opslog::SurfaceFailure` beside it — was deleted here by bl-4d81: it had no production caller after the severance, and the answer now carries each row's own `standing`, so a banner is a filter over `Query::Ops` rather than a second derivation |
| `src/app/cadence.rs` | the clock's periods (§7.2, bl-3381): the `Cadence` value, its `cadence.yaml` grammar (total parse, shared bounds), and the derived periods (wound grace, late pass, staleness). **Every derived period is spent on this side and none crosses §8.5** (bl-776a): the late pass judges this engine's own passes, the staleness threshold decides the word the workspaces answer carries, and the wound grace is waited out before a wound is stated (§7.3) — so a seat re-derives nothing out of the raw `cadence.yaml` a `ReadConfig` hands it |
...
| `src/boundary/corpus{,/ledger,/store}.rs` | the **wire conformance corpus**'s generator (REMOTE §3, §3.2; bl-32cb, bl-e598) — test-gated, shipping nothing: it renders the committed `corpus/` fixture set from the codec's own round-trip surfaces (`codec/tests/surface{,/conversation,/ball}.rs` for the request half, the reply round trip's `surface` for the answer half), so the fixtures every client replays and the fixtures yog proves itself against are ONE list. `ledger` holds `corpus/shapes.json` — per shape, every field path with the EDITION at which it appeared, plus the major, its floor and what is deprecated — which is what makes REMOTE §3.2's rule mechanical rather than remembered: a gained path is stamped the next edition and bumps nothing, and a path that vanished or re-typed is refused unless a major bump is in flight and the loss was deprecated first; the fixtures alone cannot enforce it, because regenerating them erases the diff, so the record remembers what the shapes WERE. `store` is the disk half, one renderer feeding both the gate (`corpus::gate`, an ordinary test) and the regeneration, so the two can never disagree about what the corpus should be |
| `src/boundary/{deposit,deposit/claim,consume,consume/founding,consumer,consumer/lanes,sugar}.rs` | the §8.5 deposit transport: the gestures-inbox protocol (claim-by-rename, reply files, and `mint` — the id won by an exclusive reply-slot reservation rather than guessed from a clock and a pid, bl-aa9f), with `deposit/claim` the claim **lifecycle** split out at the pre-split band (bl-d1f1) — the lock-held `Claim` whose kernel-released file lock is what tells crash debris from work in flight, the `claimed/` listing and the debris probe; one consumption pass plus the boot-time debris sweep that answers a dead claimant's gesture *in doubt* on its reply slot; its thread; the `yog gesture` deposit-and-wait sugar (`sugar/argv.rs`: its one payload — envelope or line — and the context flags a line reads its targets from, `--prepared` among them, which is how the start flow's two steps compose across two processes, bl-44d8). `consume/founding` is the one registration rule both intakes spend (REMOTE §4.1 as amended, bl-bd48/bl-0fbc): a workspace this box FOUNDS is registered to the leaves this box minted for itself, so the act a headless install actually performs — `yog gesture --ws … '/prepare'`, made by the certificate-less in-world identity — stops founding a workspace no seat can see. It hangs off `run_gesture`, the one room both intakes open onto, because a rule written at each door is a rule that drifts at one. Founding only and never joining, because revocation is a deleted file and a gesture that merely names a workspace must not write one back |
| `src/boundary/follow{,/hold,/open,/tools,/once}.rs` | the **follow lane's engine half** (REMOTE §3, §10; bl-73e7): `Query::Follow` answered as a frame *sequence* — one frame per growth of the conversation's open `response.json`, and no terminator until the stream closes. It is a **cadence, not a second reading**: whether there is a tail at all is `answer::inspector::live_tail`, bl-6233's one describer unmoved, and what this adds is where the bytes come from — the derivation folds the whole file on the worker's schedule, `open` folds the suffix on the writer's, and the two agree by `Stream::absorb`'s contract rather than by coincidence. A response file belongs to one step, so a step advancing is not an accumulator to reset but **this stream ending**, which the framing already spells; the seat then swaps to the committed entry the pull read carries and re-asks. The hold is bounded by *quiet* looks only — writing a frame is what discovers a peer that went away — and the snapshot is read off the cell per look rather than off `Deps`, because a read that deliberately outlives its request cannot be gated on a derivation frozen at connect. The **address** is still resolved once, at connect, under the caller's scope (REMOTE §4): a held read is one request. Since bl-5305 the lane's subject is the **step**, not the model call inside it — keyed on the response file it ended the instant a call settled into its `tool_use` blocks, so it was dead for exactly the minutes an operator watching an agent administer their machines needs it — and the frame carries a second half beside the prose: `tools`, the **tool window** opening and closing, read by `tools` off the step's own `tools/` subtree with a per-call watermark that is `open`'s byte offset for the other half (each minted per held connection, each starting at zero, so a dropped connection re-asks and is answered whole). Two liveness readings keep the halves honest: a driver holding the lease is what makes a step worth following, a model call streaming is what opens the prose, and between the calls of one step the response file is settled and `Query::Transcript` already carries it. Since bl-58bb a **park is a third one**: a conversation whose next invocation the capability control held has stopped driving, so every liveness reading says quiescent — and this lane answered that by ending the stream, at the exact moment the operator holding the read was the thing being waited for (on a foot lane every call to a non-shell tool is held, so that was most of the conversation). A hold is therefore not rest and is never the end of a stream; it is a window transition like the other two, said once per read off the published row's `held` mark rather than off the step's `tools/` subtree, because the seam parks the invocation *before* the executor is entered and so lands neither of the two files. It keeps its own watermark for that reason: a park is not a point on the posted→captured ladder, and an answered one runs and closes under the very same `tool_use` id. `once` is the same lane answered **once** — what an intake that cannot hold a connection gets — beside the held read rather than inside `answer/inspector`, so what one look at this lane is worth cannot be two rules; and `hold` is the **patience** — the frame vocabulary, the two numbers a quiet read is bounded by, and the parking — cut off at the cap (bl-58bb) on the seam the module doc already drew, `poll` being one look at the world taken now with no clock and no sleep in it, which is what lets a test drive the whole lane look by look |
| `src/boundary/help{,/table,/table/driving,/table/standing,/table/spend,/table/capability,/table/queries,/table/world,/table/following}.rs` | the §8.5 verb table — every gesture's usage, one-liner and page — and the one text every seat prints; the single source for refusals, `/help`, the codec's verb check and the parity tests. The roster outgrew one file at the cap (bl-dc0c) and is cut along §8.5's own taxonomy — `table` the acts on a conversation or a ball, `table/driving` the six of those whose subject is a conversation **already running** (bl-c088: send, cut off, kill, sweep, prompt again, and the §9.4 `retarget` change-of-lineage, bl-2d19 as re-scoped by bl-e654 — none of the six creates or destroys anything, and each is spelled by a `litany` run), `table/standing` the verbs whose subject is a setting, a standing policy or a record (the §4.9 monitor, the §4.3 loop, the §9 config family, the trail's own two — split off at the cap when the exit landed, on the seam where the subject changes), `table/capability` the §4.11 family's three (bl-94a5, cut off `table/standing` at the budget on the seam the family already owns everywhere else: `/answer` with its scope, and the fifth rung's `/revoke`/`/restore` over the fold it shares), `table/queries` the populating reads, `table/world` the eleven of those aimed at a workspace or the world above it (bl-c088: what exists, what each workspace holds, what its agents changed, the V4 board, and the §9 provider/client/lineage/model tables a workspace resolves — `providers` among them, bl-0164 — leaving `table/queries` the reads aimed at one conversation and the five that name nothing at all), and `table/following` the **follow-class** two (bl-73e7) — the reads whose answer is a *sequence* and whose hold is a connection thread, split on REMOTE §3's own seam rather than a line budget, since each page has to explain the same fact from two sides: an intake that can hold a connection answers many frames and one that cannot answers one, and the second is a true answer of the same question — then read back as one by `help::table()`, a function rather than a const because const slices cannot be concatenated in a const: each split is a line budget and must never become a list an operator can read half of, and each of the two bl-c088 cuts is a **prefix** rejoined where it stood, so six lists render the one roster in the one order |
| `src/boundary/line{,/args,/parse,/queries,/spell,/spell/queries,/config,/config/proposals,/verbs,/verbs/control,/balls,/enroll,/fork,/fan,/tools,/workflow,/spend}.rs` | the §8.5 **line**: the slash spelling of the boundary — the `/`-marker and its `//` escape, and the higher-order help rule read once above the verb match; the argument grammar (context reads that refuse by name, the `--flag value…` split, and — since bl-06a1 — `goal_or_prefill`, the one home of the §3.3 rule that `/prompt`'s goal is optional and falls to `prepared.goal` whole when nothing is typed: a **default, never a concatenation**, because a composer seat sends the edited whole and a `/prompt` that prepended would fire every composer-typed prefill twice); the reader — the mutating verbs, chaining to `queries`, the populating half in its own file since bl-6233 (it was a function inside `parse` from bl-0164) along §4.8's taxonomy, the same seam the codec and the help table are cut on; and the writer whose exhaustive match is the compile gate; `config` the §9 family's own grammar, reader and writer in one home, whose destination is words and whose tail is the file verbatim (bl-3f46) — and (bl-0164) whose *absent* tail, on a destination other than a lineage, is `/config`'s and `/marks`' own read; `verbs` (bl-3746) the per-verb argument builders the reader calls into — the §4.9 monitor's three and the §4.3 loop's two among them (bl-66fb) — with `verbs/control` the §4.11 capability family cut out at the budget (bl-94a5): `/answer`'s verdict and its `--scope` flag, the inverse spelling that leaves the flag off for the default, the usage string every refusal and the help page read, and the floor's two verbs; `fork` the V2 attempt's, whose flags lead so the goal can be the verbatim tail (bl-dc0c); `fan` the §3.8 mutating fan's three, beside it on the same seam — a family whose gestures read an obligation off the seat rather than a bare tail (bl-8746; `/deliver`'s summary is the verbatim tail after its handle, bl-c2bd); `balls` the `bl` family's own grammar beside them (bl-dbde) — the id-taking verbs' ball, the re-home, and the two authoring verbs whose payload is the family's own vocabulary; `tools` the routing leg's two acts (bl-024b), both of which end in a document taken verbatim to the end of the line, exactly as `/advertise`'s set does; `workflow` the §9.4 mark's two lines (bl-b680) — `/workflow <lineage>` and `/clear-workflow`, the conversation the seat's as `/retarget`'s is and the direction the verb, so a bare `/workflow` refuses naming the lineage it wanted and is never a clear |
| `src/boundary/login.rs` | **the §8.3 sign-in as a boundary act** (REMOTE §8.3, DESIGN §8.3 as amended by bl-61bf; bl-c285): `Action::Login`, which starts `bz --login --provider <row>` — in the flow that row declares (§8.3 rule 1 as amended) — on the ENGINE inside the *named* workspace's wall and answers that run's standing **without waiting for it** — a browser flow is minutes of a human's attention and the intake is one thread for the whole world, so the receipt is the standing re-read (the `Marks` discipline) and never an echo; `Query::LoginTail`, answered here once as that same standing; and `Lane`, the **second follow-class read** — everything buffered, then each arrival, then the settled exit as the last frame. **Re-ask replays** and needs no case: a lane holds nothing when it opens, so its first frame is the whole buffer, exactly as a follow reader opening at byte zero is. A pair with no run opens on one empty frame rather than on silence, because *nobody has signed in here* is a reading and not a refusal. Nothing about a child process is here — the holder is `src/login/runs.rs` |
...
| `src/budgets/{mod,bills,last}.rs` | the Usage-event vocabulary, parsed in one place (§5.1 #16) — the fold of every attempt segment beside `last`, the final reading per field, last wins (§5.1 #35, bl-9c8a: brazen's usage events are partial; its own file on the seam this row already drew), `context_window` off the same lines (the §5.1 #35 denominator, stated in band by brazen and never tabled by yog), and the **one home of the prompt reading** every figure above shares (`prompt_tokens` / `uncached_prompt_tokens` / `total_tokens`, bl-6621: the counters overlap, so a total is a fold and never a sum — and since brazen 0.0.10 `input_total_tokens` IS the prompt where a line carries it, which makes the `max` rule a per-line fallback for older records rather than the fold's rule, bl-bf8b); the one `steps/` walk every figure shares — per-step fold beside the model that billed it, the conv that owns it and its own `seq`, so a `Scope` applies after the walk and "which step is the latest" is asked in memory (§3.5, bl-9dd4; bl-a48b); since bl-53d1 a `StepBill` carries the **provider row** off the same `meta.json` read as its wall span (litany bl-4c1c, VISION §6 item 8) — the other half of the §3.5 table's key, `None` for every record written before litany said it |
...
| `src/config_edit/branch/workflow_mark.rs` | the §9.4 **workflow mark** read (bl-b680; litany ARCH §6, upstream bl-5c02) and the answer type it produces — the second per-agent fact this tree ports rather than scrapes, on `follow.rs`'s precedent exactly. `refs/litany/workflow/<id>` is litany's standing per-agent override of the one control fact that has one: nearest mark on the descent (the agent's own id, then each ancestor by `git_tree::parent_id`) names the commit whose `workflow.yaml` governs instead of the followed tip's, and the lineage is recovered from the commit rather than stored with it, `None` once the lineage has advanced past a mark that pins an older one. It rides *beside* `GoverningConfig` on the wire rather than replacing its `oid`, because a mark moves `workflow.yaml` alone and the commit-side facts still describe the tip; an unreadable mark reads as no mark, litany's own rule, and the defective workspace is refused by the governing derivation it rides with. Tests under `workflow_mark/` over real-git workspaces |
...
| `src/control/{mod,wire,classify,classify/intrinsic,classify/operand,classify/routed,bash,lex,rules,rules/table,policy,hold,reason,root,judge,judge/answers,judge/table,author,ceiling}.rs` | the §8.6 capability control (VISION §4.11): the consult a `world/tools/` shim runs, and the one sentence a park hands the operator — tool, bounded input summary, class, evidence; litany's two wire shapes; the effect vocabulary, and under it the three files that are the classification itself (bl-72bd): `classify/intrinsic` the **closed** set of names this control implements a row for, folded into an enum before anything classifies so the match over it is exhaustive and a name added without a class does not compile; `classify/operand` the two rows (`cd`, `apply_patch`) judged against the writable root at consult time; `classify/routed` the fail-closed lane every other name takes — a command line is classified by the bash ruleset exactly as the engine's own `bash` is, against a root whose writable set is **empty** (bl-1772: this control vouches for no path on a machine it cannot see, so a `cd … &&` chain and the absolute spelling of the same act classify alike); an input carrying none is classified by the operator's own `rules:` row for the **whole host-qualified name** when they wrote one (bl-b65d — the way an MCP tool, whose input its server's schema shaped, stops holding on every call), else by the address its input names — a `url` or `urls` is open-world, that class's own definition being network reach (bl-c6f0), read after the row and the line so neither is softened and never answering narrower than open-world — and anything else is `opaque`, the one class the shipped table holds, whose sentence spells the row that would end it. There is no arm from a tool NAME to a passing class; the bash ruleset over every program a command runs; the shell lexer that finds them; the grammar one rule is written in, with `rules/table` the shipped ruleset as data — one list, because first-match-wins makes its order the policy; `reason` the sentence itself in **both** directions (bl-94a5) — the hold's text and the refusal's added paragraph on one side, and `class_of` reading a class back out of that text on the other, one file because the mark carries three fields and the reason is the one of them yog wrote, so a widened answer learns which class it releases without asking upstream for a fourth; `policy` the per-workspace override that ruleset is the default of — `capability.yaml` at the live config tip, four keys, absence *is* the defaults (bl-765d) — whose `rules:` rows are keyed on a routed tool NAME as well as on a program (bl-b65d), `stated` being the name lookup and the operator's rows the only ones it reads; `hold` litany's valued hold mark, read one agent at a time by the answer gesture and whole-namespace by the snapshot tick; the writable root and its lexical containment; the class→verdict table folded with the trail's answers and floors, and **addressed to the leg the name runs on** (bl-1772: on a routed leg a refusal becomes a hold, a refusal being a sentence handed to the one party that can rephrase it, while the operator's own once-answer stands ahead of the mapping); `author` the workflow fixed point that makes a workspace born adjudicated **and born unbounded** — one pass over `workflow.yaml` that authors the `tool_control:` block and strips litany's whole-tree `budgets:` ceiling (bl-56af: §3.5's dollar ceiling is the one that survives, and a template only reaches workspaces born after it); its *drive* is `start::ensure`'s single convergence, shared with §3.7's manifest glob; and `ceiling` the §3.5 spend ceiling's **second seat** (bl-4b48, §8.6's last bullet) — `ui.json`'s `ceiling` and `prices` read through the §7.1 address book that already spells that file's name, the §3.1 roster folded by `spend::of_world`, and `Ceiling::verdict`'s own sentence handed back unaltered so the `/ceiling` release selects what the ceiling parked by the fixed head it wrote. Resolved **once per consult** onto `Consult::ceiling` and judged as a value, ahead of the table, the floor and the class — the one thing ahead of *it* being a once-answer to this exact `tool_use` id (`Answers::once`, the precedence the floor already states). `Ceiling::armed` is what keeps an unbounded or unpriced world at one `ui.json` read: the roster is not enumerated and no `steps/` tree is walked; `hold::all` (bl-53d1) enumerates the whole `refs/litany/held/` namespace of one workspace live, with the agent each mark parks — the §3.5 ceiling release's read, kept beside the one-mark read so both parse one shape |
...
| `src/dht.rs` | **the REMOTE §13.2 DHT rendezvous client** (bl-df31): a pure client of the mainline DHT and never a node — no listener, no routing answers, no storage offered. The root is the interface the punched-wire chain consumes: `Dht::new` takes a `Box<dyn Transport>`, the bootstrap addresses and a `Config` (α the window, K, the per-query deadline, a query cap — every duration a test can shorten), and answers `lookup` (the nodes nearest an id), `get` and `put` — and `observed` (bl-efae), where the last walk's answering nodes said they saw this client, voted per address family: the endpoint most of them named, nothing on a tie. The claims are kept raw and the vote is computed on each ask. The client's own id is random per run because nobody routes by it. Its corpus runs the whole client through the real UDP transport against a fake DHT the suite stands up on loopback, one thread per fake node, with a mood for each way a node on the commons misbehaves (REMOTE §13.5), `Router` among them (a bootstrap that answers `find_node` and is silent to `get` and `put`, as the mainline's are), `Rotor` the measured one (a `Router` naming one peer eight times per answer, the next peer on the next query — bl-d00f), `Claim` too: a node that answers and asserts a BEP 42 `ip` of the test's choosing, true or not. The fake's datagram builders (`tests/fake/wire.rs`) sit apart from its store |
| `src/dht/bencode.rs` | bencode (BEP 3) in-house — one enum, one canonical encoder, one strict decoder (REMOTE §13.2: zero new crates). Strict where a hostile datagram makes it load-bearing: the whole input is one value, nesting is bounded, a length past the end refuses. Canonical because BEP 44 signs a canonical bencoding |
| `src/dht/flight.rs` | the window's primitive, split from `lookup` (bl-d00f as `round.rs`, reworked bl-d9c1): `ask` sends one query and puts its transaction in the air with its own deadline only if the send went; `land` waits for ONE event — an answer to a transaction in the air, which leaves it, or the nearest deadline passing, every expired query leaving — reading the socket with a timeout equal to the nearest outstanding deadline and reading past noise and answers to transactions not in the air. `lookup` refills the window on every event; `items` drains it for `put` |
| `src/dht/frontier.rs` | a walk's state and the frontier read off it (bl-d9c1, split from `lookup`): the pool keyed `(distance, address)` so a repeated entry is learned once and one id at two addresses is two nodes, the asked and replied sets, the outcome and the BEP 42 claims, and the door addresses that answered — the only ones a re-ask knocks at (bl-f519). The frontier is the K closest nodes that replied, are not yet asked, or are still in the air, so a node silent past its deadline, one that answered only an error, and one the socket refused all leave it (bl-d00f); it answers the next node to ask, whether a frontier node is still in the air, how many walk queries hold α slots and whether a door query is out. The door's reply seeds the pool and its claim votes, but the door is never a result (bl-9408) |
| `src/dht/items.rs` | the two BEP 44 verbs over the walk: `get` is the newest item under a key and salt that VERIFIES — an unsigned or forged item is nothing, never a value — and `put` is the same `get` walk (it is the walk that collects write tokens; the bootstrap is asked `find_node`, never `get`, because the mainline's routers never answer `get` — bl-f6e1) followed by one flight of `put` to the K closest token holders, each with its own deadline, answering how many acknowledged, with zero an error naming the first refusal — and a walk that reached no token holder at all its own error, sending nothing (bl-f519) |
| `src/dht/krpc.rs` | KRPC (BEP 5), the client's half only: the query datagram, the reply and error read back, a reply's BEP 42 `ip` (the querier's address as that node saw it; six or eighteen compact bytes, any other length nothing), compact `nodes`/`nodes6` parsing with a ragged tail dropped (one compact-address reader serves both), and the 160-bit `NodeId` with its XOR distance. A query arriving is `None` — this end answers nobody |
| `src/dht/lookup.rs` | the iterative walk, as a sliding window (bl-d9c1): up to α walk queries in the air, each with its own deadline, and the next frontier node asked the moment any of them answers or times out — so a silent node costs its own slot for one deadline and never delays an answer beside it. It converges until the K closest RESPONSIVE nodes have all been asked (bl-d00f); the frontier itself is `frontier.rs`. A frontier that runs dry short of K replies asks the bootstrap again while the bootstrap has named anyone (the one live router names one random node per query), knocking again only at the bootstrap addresses that answered — a silent router leaves the door for the walk (bl-f519); the door's queries hold no α slot, and a door ask always spends at least one query of the cap. Bounded twice against a hostile commons — a query ends at its deadline, the walk at `max_queries` however many closer nodes the answers invent — and a walk ends without waiting out a query no longer on the frontier. The bootstrap is always asked `find_node` and every other node the walk's own verb, so the BEP 44 verbs reach nodes near the target rather than the routers, which never answer `get` (bl-f6e1). A walk whose learned nodes were all silent is the dark-commons `Err` (bl-9408). The walk's answering nodes' `ip` claims replace the last walk's for `Dht::observed`, so a walk that heard nobody observes nothing |
| `src/dht/mutable.rs` | BEP 44's mutable item and the keypair that signs it — ed25519 and the target's SHA-1 both from `ring`, the provider rustls already links (AGENTS.md rule 6: no new crate). The signed bytes are the BEP's exact spelling, `4:salt…3:seqi…e1:v…` without the enclosing dictionary; the value is always a byte string because both of §13.2's items are sealed opaque bytes; a value over the BEP's 1000-byte cap is refused before it leaves |
| `src/dht/transport.rs` | the datagram seam: the `Transport` trait and the std UDP socket that fills it in production — ephemeral port, read timeouts, no listener semantics, no tokio (AGENTS.md rule 8). A trait so the same client runs against loopback fakes through the real socket AND against stand-ins for the failures loopback will not produce on demand |
| `src/doctor{,/engine,/workspace}.rs` | **is this box wired up** (bl-28f4) — the one read that asks every check a first conversation depends on at once, and names the act that fixes each one it can. `engine` is the four a box with no workspace still has (the wire material, the endpoint `wire/address` names, what this process actually BOUND — which the file cannot say, since a `:0` is a request only the listener learns the answer to — and the git identity every step is committed under); `workspace` is the two a named workspace adds (whether a conversation started there would reach a model, asked by the `Prompt` door's own gate and quoting its refusal verbatim, and who is registered to reach it). **Every fact is already derived somewhere and none is re-judged here**: a doctor with its own opinion about a wall is a second authority for a decision the door already makes, and the second one drifts. It reads and never writes — a diagnostic must work on a box broken in the way it is diagnosing — and answers no tally, a seat counting the rows it was handed rather than being told a number that would disagree the first time a row was added |
...
| `src/fixture{.rs,/recipe.rs,/roster.rs,/places.rs,/disk.rs,/lay.rs,/verb.rs}` | **fixture worlds** (bl-8741): named, deterministic world states an external client harness can dial and read. `recipe` is the vocabulary a state is written in — a conversation, its `refs/litany/*` marks, and the shape of its newest step — all `&'static`, because the whole promise is that two lays of one name write the same bytes and data that cannot be computed cannot drift; `roster` is the states themselves and the whole of what a consumer may ask for, each written as a departure from the empty one (§3.4's own posture, so a first run is no more a special case here than in the engine); `lay` is the writer, deriving every path through `world::layout_under`, `binding::names_root` and the §16.2 anchors rather than spelling one, dating every commit and mtime from the recipe's own offsets, and pinning the `config/default` root at a **fixed** instant so the trunk's oid is byte-identical across runs, and committing onto that root the `providers.yaml` role assignment a `litany new` trunk carries (bl-59cf) — without it a laid world declared no role at all, so the three §9.4 surfaces a seat tunes with answered nothing: `/roles` an empty table, `/effort` and `/priority` the block grammar's *no such entry*, which is a refusal no harness can clear from outside the world the verb has just wiped and rebuilt. The pointer names the row `Recipe::brazen` declares and carries **no credential**, which is the whole of what the refusal-free path needs: the tuning pair reads no provider table at all, a capability deciding which control a seat offers and never whether a write is allowed; `verb` is `yog fixture`, shaped like the mint above it (pure plan, environment readings, the process edge reads) and carrying the guard a destructive act owes — a root overlapping this box's yog data root in either direction is refused before anything is removed. It **lays and prints; it does not boot**: `XDG_DATA_HOME=<root> yog` is the boot and the caller owns that process because the caller must kill it. Two premises the tree corrects and the module doc records: a *speaking* conversation is an open fd and a held lock (§3.5), never a tree, so the answer names the descriptors a harness holds instead of pretending; and `wound_grace` is the **server's** (bl-776a), so a laid step must be dated past it — a `request.json` left at "now" would answer as a call in flight and no state would keep its promise, which is why the request's mtime is stamped from the recipe's offsets beside the response's |
...
| `src/fork/{mod,choices}.rs` | the V2 **attempt** (bl-dc0c): `mod` what one fork *is* — the `litany dispatch` argv and the skill pins; `choices` what this world makes **available** to one — the roles the config governing a ref binds to a model (§5.1 #34), and the world's skill-pool path, derived on demand and stored nowhere. **What one seat *offers* is deleted** (bl-7cc8): the fork-point list, the pool listing, the `Choices` fold over them and the `composer`'s ×N and readiness rule. `Action::Fork` carries one attempt by ruling (§8.5) and is fired N times, and no reply carries an offer, so the arithmetic and the offer are both a seat's. Nothing here can touch a project worktree — the rung is read-only by construction (VISION §4.10, bl-2b8c) |
...
| `src/git_tree/{streaming,streaming/wire,tool_event}.rs` | the live `response.json` fold — **one read, two facts** (§5.1 #10, #28b): the display text every tail seat reads, and the last content delta's kind that splits an open model call into waiting / thinking / inference; with `wire` the fold's own JSON spelling both directions (bl-73e7), beside the type for the reason `transcript::wire` and `rail::wire` are — the shape of a fold is the folding module's vocabulary, and the follow lane's frame body is that fold and — since bl-5305 — the tool window beside it. `tool_event` is that second half's datum and its spelling (REMOTE §5.5): one opening or closing of a tool call, read off litany's per-call `input.json` / `output.json` pair, whose two existences *are* the window's two transitions. Its capture status is the exit code's **presence**, not a second field, so "complete, status unknown" is not an arm anything can be in; since bl-58bb it carries `held` on that same discipline — the control's reason for parking a call, whose presence is the park and whose source is the hold mark rather than a file, since a held invocation lands no `input.json` at all; the closing entry restates neither name nor input, because a follower holds the opening one keyed by the same `tool_use`; and the machine a routed call ran on is already in the recorded name, REMOTE §5.1 presenting a loaded remote tool as `<client>_<tool>` always — splitting that composition back apart would be a second reader of a rule with one home, and asking the registry instead answers *where it would route now* rather than where it ran |
...
| `{src/multiplex.rs,src/multiplex/bl.rs,src/multiplex/litany.rs,src/multiplex/help.rs,src/multiplex/landing.rs,src/multiplex/namespace.rs}` | the §16.7 namespace arms: each embedded crate's verb surface, dispatched from `main.rs` — plus the router's namespace table and its exhaustive `owns_argv` classification (`namespace`, bl-4667 — which arms own their argv and answer `--help` themselves, and which are answered from the command table like `serve`); `help` (bl-52ed): the argv seat's whole command table, the top-level roster rendered from it, every per-command page, and the discovery probe the `bl`/`bz` arms answer world-free (§8.5's every-command-answers-help rule at the argv surface); and `landing` (bl-7e54): the §16.3 repair the `bl` arm converges on the way in, re-deriving a pre-nesting landing's plugin schedule from balls' own seed — its on-disk half rides `tests/multiplex_landing.rs`, since founding a landing forks inside balls (bl-6bf5), leaving the error plumbing alone beside the code; and `wire` (bl-b6fa, bl-024b): the two wire CLIENT arms — `yog seat` and `yog tool-host` — which compose the world at the process edge because the certificate and the tool config are facts of *this* machine's data root even when the engine is elsewhere |
...
| `src/proposals{,/wire}.rs` | **the learning loop's operator half** (§9.6; REMOTE §9.22, bl-dd88): the staged proposals a reviewer left, one of them whole, and the settle that accepts or rejects one — with `wire` the answer's own spelling both ways, beside the type that owns it as `transcript::wire` and `rail::wire` sit beside theirs. **The read is yog's and the act is litany's.** yog already reads this workspace's other two ref namespaces itself — `agents/*` for the §7.1 tree, `config/*` for the §9.3 browse — so a listing over `proposal/*` is that same derivation through the same scrubbed doorway, and it answers typed rows rather than a rendered table a seat would have to parse back; the settle is a compare-and-swap fast-forward whose expected old value is the freshness the listing showed, and re-implementing that here would be a second home for a rule whose failure mode is a lost race, so it stays a workspace-bound `litany proposal` and leaves that family's §4.2 row. Freshness is derived at read time and stored nowhere: a row's `fresh` is its parent compared against the config heads in the same pass, and the accept re-derives the same test as git's own `update-ref <head> <new> <old>` |
| `src/rail/{mod,cards,cohort,pin,place,tree,wire}.rs` | the **step spine** (VISION V1, bl-98da; re-seated into the chat by bl-1802), cut on five seams and all derivation: `mod` the spine's *shape* — the notch spine over the Steps view's `meta.commit` (§5.1 #29) and the row→notch lookup a seat threads its chat through; `place` where each notch sits among the transcript's rows and how far its pin cuts, pairing one sealed model-output entry to each completed step (the ordinal alignment bl-929d and bl-98da both got wrong); `cards` a child's *placement* on it — the shared-commit-prefix fork point, the two edges, the fork label and the streaming tail, all pure over facts the snapshot already carries; `pin` the fold that threads one notch through the inspector (the transcript prefix, the budget as-of); `tree` the only new disk read — the Files tab out of `ls-tree`/`git show` at the pinned commit; `cohort` V2's fan, which is nothing but those cards grouped by the notch they were born at (§5.1 #33); and `wire` the spine's §8.5 spelling both ways, beside the type for the reason `workdiff::wire` gives — notches, seats and cards are this module's own vocabulary (bl-6233, decode bl-7067); since bl-53d1 a notch carries the rollup **priced** beside the token one (`cost`, the §3.5 figure folded over each step's own cost through the notch, absent for an unpriced world), and `wire` spells it through `reply/cost` |
| `src/registry{,/enroll,/leaf,/mailbox,/mailbox/doubt,/mailbox/slots,/mailbox/slots/read,/peer,/presence,/roster,/seen,/tools}.rs` | the REMOTE §4 client registry (bl-8bbc): one **file per registration** under `<yog-state-root>/clients/<client>/workspaces/<name>`, so registering is a write, revocation is a delete and the registered set is a listing — nothing stored that the path already says. Plus the reserved `local` identity every in-world caller (window, deposit inbox) owns a directory under, and `leaf` — the subject common name read off a peer's certificate by a structural DER walk, because yog links no certificate library and a byte search for the CN object identifier would return the issuer's name, which comes first. REMOTE §5's two facts about a tool host hang off the same directory (bl-4e08) and they are deliberately different KINDS of thing: `tools` is the durable half — `clients/<client>/tools.json`, one document per client rather than one per registration, because a tool set is a fact about a machine and the registration listing already says which workspaces see it — carrying the ONE spelling of an element (name, description, JSON Schema verbatim) that the boundary codec and the file both spend, plus the write-only-when-it-differs store and the two ways a presentation declines; `presence` is the live half — one entry per live connection behind the `state.rs` alias, entered by the listener as an RAII guard so there is no leave verb to forget, and never a file, since presence changes at connection rate. Since bl-1be7 each entry carries the corpus **edition** that connection stated in its preface (REMOTE §3.2), so the count is the list's length rather than a second number to keep true: capability is a fact about a connection, and this is where a connection's identity already lives. The read answers the whole list and not a summary — two seats of one client may be two builds, and picking the newest would over-promise a call landing on the older — so the judgement waits for the reader that needs one (`reply/clients` is where it would go, and does not carry it yet); and `roster` joins the three reads at the moment they are asked (listing, presence, advertised set) so every seat is handed identical rows, with the reserved `local` name filtered by the rule that already refuses it rather than by a second case. `mailbox` is REMOTE §5's third fact about a tool host (bl-024b): the vocabulary of a routed invocation — the call, the invocation, the capture, the completion, and the `Verb` that folds the two acts — plus its ONE JSON spelling, spent by the gesture codec, by both replies that carry a capture and by the client-side executor alike; `mailbox/slots` is where in-flight ones live, a queue per client and a slot per invocation behind this leg's one lock (the fourth `rules/locks-outside-state.yml` carve-out, for `presence`'s measured reason), swept an hour after a post so a driver that died mid-invocation costs one entry rather than a leak. It also holds **which identities are parked on a follow-class read** (bl-1462): one reader per client, an RAII claim rather than presence's refcount, because two connections on one machine's queue is the pathology itself where two on its presence is an operator with two seats — the second read is refused in band, and so is an advertisement that would change a serving machine's set out from under it (REMOTE §5.1). `mailbox/slots/read` is the follow-class read itself, cut off the map at the §12 budget on the seam the two halves already had (bl-8016): everything there is a slot's own life — the post, the completion, the collect, the hour sweep — and everything here is a *read in progress*, which parks, holds its one-reader claim for exactly its own duration, and decides what a client is handed. The hand-off mark it leaves is a **lease and not a latch** (bl-e658, REMOTE §5.3): a read parks for the whole hold and cannot learn its peer went away, so a slot handed to a read that never answered it goes out again at that client's next follow-class read — the leg is at-least-once, and the invocation id it is redelivered under is the idempotency key. **The lease is bounded and the exhausted one is answered** (REMOTE §5.6, bl-8016): the hand-off COUNT is the slot's one stored fact about it and "handed to the read running now" is derived per look rather than stored beside it, three hand-offs is the whole lease, and at the read that would hand a slot a fourth time the engine writes its capture itself — non-zero, naming the client and the count, instructing a read of the world rather than a re-send, exactly as the §8.5 gesture sweep does. A late completion overwrites an uncollected one, because a real capture beats a doubt. The map stays RAM across a restart, so the spent-handle sentence names that as its third cause. `mailbox/doubt` is where those sentences live (bl-0655), cut off the vocabulary on the seam that everything else there is a *shape* — what an invocation is, what a capture is, how each is spelled — while these three are the engine's own prose, the only text on this leg a model or an operator reads that no tool produced: the unheld-handle refusal, the exhausted lease's capture, and the mark a **redelivered** capture carries. That last is REMOTE §5.6's named exit, taken: below the cap a hand-off count above one is said on the capture's stderr at `complete`, where the count and the real capture meet, because litany renders a tool result from exit code, stdout and stderr and nothing else — a field would be a fact only a decoder could read, on the lane whose reader is a model. All three open on one clause, so the count has one home and only the endings differ. `peer` is REMOTE §4.2's grade (bl-7ff3): the two values a leaf's subject can carry, the closed set of gestures a foot may say (advertise, take its invocations, complete one — never `invoke`, the asking side's), and the `Peer` an intake answers as. It is a value BESIDE the identity rather than a field on it, because `Client` keys the presence map, the mailbox and every registration on disk: a peer that connected under one grade must not be a different key from the same client read off the `clients/` listing. Default-operator is total rather than defaulted — `leaf::grade` answers a `Grade` and not an `Option`, so a certificate minted before the grade existed, or bytes that are no certificate at all, read as operator. `enroll` is REMOTE §1.4's two values (bl-f4e3) — what an operator asks for when a device joins (a workspace to seat it in, the common name its certificate will carry, the grade that certificate may say) and the whole of what the engine answers with (that grade and name, the address clients dial, and the three PEMs). They live here rather than at the boundary because the identity the act mints and the registration it seats are the registry's own two facts, and because a payload type with its own module is the fold `mailbox::Verb` and the monitor's family already take: ONE variant at the boundary, one home for the ruling. The private key exists in the `Enrolled` value and nowhere else on this box — the executor shreds it before the answer leaves, and the certificate stays precisely because its presence is what makes a second enrollment under one name an adoption or a refusal rather than a re-issue. `seen` is the THIRD durable fact a registration carries (bl-d542): `clients/<client>/seen`, the unix second that client last spoke, written at the wire intake per request and read back onto the roster row — because `present` alone could not tell a machine asleep right now from one that never once connected, and on a terminal seat it reads false for everything. It is best-effort by construction: a stamp that cannot be written costs a column, never a gesture |
| `src/science/{mod,bound,observed,outcome,wire}.rs` | the §3.9 **attempt science projection** (VISION §4.10 item 7, bl-40ab), cut on four seams and owning no fact of its own: `mod` the join — the row type and the one function that assembles it, over `workdiff::read`'s own row set so *which attempts are there* has one answer, and each row *carrying* the `workdiff::Attempt` it composes rather than restating its identity, refs, OIDs, churn and acceptance mark; `bound` *which* conversation an attempt is bound to — the one binding rule at every N (the last fire whose `--cwd` names this attempt's worktree, by balls' own `attempt_path`/`work_worktree_path` formulas) and the §3.3 name→id resolution; `observed` *what about it* — the fork's own inputs (`goal.md`) beside §5.1 #17's **followed** config commit, which since bl-e654 is a fact about now rather than a frozen one (§3.9), the terminal response and the delivered messages with the compacted bound beside them (how many entries the counter proves deleted, bl-fde5 — the §3.9 statement that both were read over a rewritten record), and the step-record columns as an in-memory filter of `Snapshot::bills`, so the projection makes no second pass over `steps/`; `outcome` the four arms over three git facts — the derived acceptance mark, whether the source ref still resolves, and `merge-base --is-ancestor`, which is the delivery law's own staleness precondition read from outside and this ball's reframe of *"the source advanced after a refusal"* — plus, beside them because it is the same question in the same shape, the **base** commit the two ends departed from (item 7's third OID, balls' `merge-base(target, source)` formula spelled here because asking balls means resuming an attempt, and resuming writes); both ancestry calls are `git_tree::cmd`'s existing ones, never a second spelling; `wire` its §8.5 JSON shape both ways, beside the type for `workdiff::wire`'s reason, with the diff column spelled by that module's own row codec; since bl-53d1 `observed` prices the same bills the usage column reads (`cost`, absent for an unpriced world), so the projection takes the table beside the snapshot and `wire` spells the figure through `reply/cost` |
...
| `src/spend/{mod,prices,prices/parse,ceiling}.rs` | the §3.5 join, pure over the worker's pre-walked bills (bl-9dd4) — selection, attribution, the honest-granularity label, and the unpriced remainder, with `of_world` the one deliberate fresh walk because a gate compares against now; the price table **keyed by `(provider row, model)` with a `*` row** (bl-53d1): `prices` the table, its one lookup — exact, else the row's `*`, else unpriced, and with no provider named the same rule with the filter empty, unique-by-model or ambiguous — its flattening for the wire and the `set` a `/price` act spends, the old model-keyed document reading empty under the forgiving read; `prices/parse` split off at the pre-split band when the table gained its dimension — one rate's shape, the forgiving read of one quoted figure, the micro-USD arithmetic over the §3.5 three-way partition of the prompt (bl-6621 — the cached slice is priced once, and the tokens priced sum to §5.1 #16's fold exactly), and `decimal`, the one rendering that writes a micro-USD figure back as the decimal the operator typed, which is what `ui.json`, the `prices` reply and the line all spell; the §3.5 spend ceiling's policy half — the operator's number and the at-or-over comparison against the world figure (bl-56d5), and `CEILING_HEAD`, the fixed head of every sentence it writes, which the release reads back off a hold mark (bl-53d1); the one figure every spend answer carries — the board's ball rows and the conversation's settings rows (bl-2e18) — whose attribution clause is independent of the price table, so the honest-granularity label survives deleting the cost column (bl-1765) |
| `src/start/{mod,model,goal,identity,exec,exec/claim,ensure,grant,grant/keep,prompt,run}.rs` | the start flow (§3.4/§8.1): pure plan, `model` the inert shapes it reads and returns (`Payload`/`BallSpec`, `StartInputs`, `Step`) cut off `mod.rs` at the §12 pre-split band — what a start *is* beside the one function that derives it, goal compose, the §3.3 stamp and its inverses (the pre-mint name **preview** and its `Composer` view-model are deleted, bl-7cc8: `Prepared` carries no predicted name, so no reply could state one and the wording of a prediction is the seat's; the §8.1 `StartGate`/`WallCredit` fold went the same way, its inputs already crossing as `Query::Providers`), the `bl`-facing gated executors, `exec/claim` the one of them whose *answer* is still judged (bl printed a worktree path; the bl-delivery formula says whether it is the canonical leaf, the `<id>-<claimant>` variant, or a `Drift` logged before it returns), `grant` the §8.6 birth grant (bl-0460) — the one name yog's own `clients` tool needs in the worker's `providers.yaml` list, which litany's shipped template cannot carry because the tool is not litany's, **with the `descriptions/tools/clients.json` it is worthless without** (bl-7d33: litany refuses a fork whose grant its config commit does not describe, so writing the grant alone stopped every conversation on the tree), converged in the same pass as the control block and the instruction glob and never onto the checkpoint roles — and, since bl-d281, the **prune** beside it: a grant is durable and a pin is not, so a name the pinned engine no longer ships is removed rather than left to classify `opaque` and park every call of it, with `grant/keep` the whole predicate (`litany::cmd::BUILTIN_TOOLS` plus yog's engine acts and `clients`, widened only by an operator's own `capability.yaml` `rules:` row, and never a list written here); `ensure` the workspace's existence and its policy — `litany new` **into an I3 temp beside the destination, renamed onto the name only once litany has finished** (bl-1af5: a failure part way used to leave a `repo.git` that enumerates as a workspace, which wedged the name forever behind an `unknown workspace` sentence about addressing) plus the one convergence that authors §8.6's control block and §3.7's instruction glob onto **the lineage the drone will fork off** (§8.7, `config/default` unless the ball's tags named another) in a single `litany config` drive, outside the create skip; the §9.2 birth-template gate that once sat here is retired (bl-00ee) — the detached fire, whose one argv carries `--name`, `--config`, `--cwd` and every `--pin` |
...
| `src/steps_view/{mod,cost,detail,wound,wire,wire/decode}.rs` | the step inspector — `cost` the row's money (bl-53d1): each summary's own bill priced by its own `(provider, model)`, joined off `Snapshot::bills` by conversation and sequence rather than by a second walk, absent for a step the walk has not billed —, incl. the §7.3 wound in both its classes — no response, and the §4.4 output limit (§11 Steps). `mod`'s `build` is where the §7.2 **catch-up window** is spent (bl-776a): a no-response wound is stated only once the step's own call start has outlived the live cadence's `wound_grace`, so the wound crosses already-judged and no seat times one. `wound` itself stays pure and Clock-free — the clock is the builder's. `mod`+`detail` are the list and drill-in **reads**; the paints and the column table that went with them are the seat's (bl-7942), and the rule that table enforced survives as this crate's: **no field is answered without the name that says what it is** (bl-3ffc), which is what `wire` spells. `wire` is the §8.5 spelling of **both** tiers, cut along that same read seam (bl-6233), with `wire/decode` its other direction at the §12 budget (bl-7067) — also the one home of the `BudgetSpend` shape, which the §3.5 board figure spends rather than keeping a second wording of four counters |
...
| `src/steps_view/seq.rs` | **which record directory a step sequence names** (bl-136f): the one join between the number an operator types and the zero-padded directory litany wrote (`001`, ARCH §2.3). A numeric seq is read as the number it is and re-spelled at the record width, so `3`, `03` and `003` are one address; a seq that names no directory **refuses, naming the ones that do**. The listing it quotes is `mod`'s own `step_seqs`, so what a seat is told exists is what the Steps list would have shown it. It exists because `Doc::of_file` on a missing file is `Absent` by design — the classification that tells an empty record from a malformed one — so a wrong path answered `ok:true` with every record absent, a false negative on the audit path |
...
| `src/test_support/fixture.rs` | the suite's door onto `git_env::write_exec` — that function with the error turned into a panic, which is all a fixture wants (bl-fd28, bl-e6c9). The hazard, the measurements and the rejected shapes are stated once, where the write happens. The two narrow `read_only`/`writable` helpers beside it are the OTHER fixture ("this file cannot be rewritten") and expose no mode parameter, which is why this file is the one `rules/no-hand-chmod.yml` still exempts in `test_support` |
...
| `src/tool_host/engine_act.rs` | **the acts whose subject is the conversation, performed as engine acts** (REMOTE §5.4, bl-dfce): the compactor's procedure pair `write_summary` and `mark_for_deletion`. litany injects tool definitions from two sources — the host's injection and the calling role's own procedure — and since the router went total the second source arrives here too. They are not tools on a machine: their subject is the conversation, which the server holds, so the subject-locality invariant sends them nowhere and yog answers them itself. **litany defines what they do and yog restates none of it** — the acts are performed by re-entering the engine's own front door (`<driver_target> tool <name>`, the third hop litany's resolution addressed before the inversion) with the caller identity on the child's environment and the `tool_use` input on its stdin, so the compactor's semantics have one definition and it is upstream's. **Ten rows since bl-9ced**: the pair, the conversation-subject worker grants (`dispatch`, `message`, `load_skill`, `cd`), and the four acts on the agent's own record, history and lineage (`python`, whose program composes the agent's tools by re-entering the front door and whose inner records land under the in-flight step; `search_history`, a pickaxe over the workspace's `agents/*` refs, which a foot holds none of; `remember`, which commits a staged `proposal/<agent-id>` branch on the workspace repository; and `read_tool_output`, which pages the agent's own `steps/**/tools/<tool-id>/output.json` capture back out under a domain bound that is one string equality against its own `LITANY_CONV_BRANCH`, and which a foot holding no `steps/` tree could only answer *no such address* to) — each admitted by the same subject-locality audit and performed by the same re-entry, which since bl-fe43 also carries `LITANY_TOOL_ID`, the invocation's own id, so a child's record directory and the id it reads cannot disagree — at the caller's resolved cwd since litany bl-ddaa, the engine's own contract for in-process built-ins. The name set is closed and enumerated here and nowhere else — the worktree lane's last rung (bl-5710) calls this module's `perform` without joining the set, because what separates the two is the ordering, not the mechanism; both waits are bounded (the stop flag, and the patience the tool bound already is), and a failure is the in-band non-zero refusal every other answer on this seam is |
| `src/tool_host/grant.rs` | **the calling role's `providers.yaml` grant** (REMOTE §5.2, bl-52b7) — the gate on the whole injection, and two reads with one existing home each: the agent's **role** off litany's dispatch commit subject (`dispatch: <role> [<agent-id>]`, its ARCH §2.5 — roots lack the prefix and roots are workers, litany's own default rather than a case), and `roles.<role>.tools` out of the `providers.yaml` of the config that **governs** that agent, through §9.4's block grammar so the picker, the fork composer and this reader cannot disagree about what a config file says. yog stores neither fact. **Every unreadable one answers *granted nothing***: an absent workspace, a repository git will not read, a role the file does not declare, a `tools:` value that is not the flow sequence litany writes — because the opposite default, reading an unreadable config as full trust, is the defect this file exists to close |
...
| `src/ui_state/{prices,ceiling}.rs` | the §4.1 `prices` object: the §3.5 price table's one read, forgiving, **and since bl-53d1 its one write** — `set_prices`, the same write-through `set_pinned` takes, deleting the key rather than storing `{}` when the last row goes; the §4.1 `ceiling` number beside it, read the same way — absent is no gate (bl-56d5) — and written the same way (`set_ceiling`, `None` deleting the key). The *read-only, setter-free* ruling stood while the window ran on the engine's box; REMOTE §12 put the seat on another machine, and the door is `Action::Price`/`Action::Ceiling` (§3.5) |
...
| `src/wire/hello{,/version,/version/exact}.rs` | the wire's **version preface** (REMOTE §3, §3.2, §12; bl-a670, bl-e598): each end writes one frame stating its protocol version before it reads the peer's, so neither waits on the other and a skew is nameable from whichever side notices. A mismatch is fail-closed — the engine refuses in band on the connection the peer opened, the seat refuses to its caller as the one `Err(String)` every transport failure already arrives as, and both say the same sentence naming BOTH versions, because with four separately installed components (REMOTE §12) the refusal IS the upgrade prompt. No negotiation, no capability probe, no compat shim, and no change to the request frame: the preface rides beside the gesture envelope, so the frame the wire carries is still byte for byte the frame the `gestures/` inbox carries. ALPN would have cost no frame and refused inside rustls, but a TLS alert cannot name a version — which is the requirement. **The number lives in the repo-root `PROTOCOL` file and its changelog in `version`** (bl-94a5 split the file, bl-3e57 moved the number out of Rust entirely): `build.rs` compiles that one line into the constant `version` includes, so there is one copy and nothing to hold equal to it, and four repositories' release gates fetch the number at an address no module split can move — the split itself had already broken every one of them (REMOTE §3, thrall bl-c618). `hello` is the preface exchange, `version` is every reason the number has moved. Since bl-9ced `version` carries a SECOND integer no preface ever states: `PROTOCOL_PUBLISHED`, the newest version yog has published and therefore the newest a peer can be speaking. Its one reader is the corpus ledger's rule that a wire shape may not change at a version already in use — judged against the published floor rather than against the version the record was last generated at, so a wave of lanes shares one unreleased number instead of costing one integer apiece, which under REMOTE §3's release hold is three consumer repositories re-vendoring a constant once instead of three times (REMOTE §9.11 as amended, §9.21). The pair is the `<n> → <m>` heading every ledger entry already writes, made readable by a program. **Since bl-e598 the number is a MAJOR** (REMOTE §3.2): it moves on a breaking change only, an addition is an edition the corpus ledger stamps, and the exact era's entries (4 → 18) moved whole to `version/exact.rs` — the era seam the file named for itself — leaving `version` the discipline, the `18 → 19` entry and the two constants. **Since bl-1be7 the preface carries a second key, `edition`** (REMOTE §3.2): `build.rs` reads `corpus/shapes.json` beside `PROTOCOL` and writes `EDITION` — the newest stamp over every shape's signature, computed there exactly as `Ledger::edition` computes it and held equal to it by a unit test on the committed record — and `FLOOR`, the record's own. Both ends write it and `admit` never adjudicates it: an edition differs only WITHIN an agreed major, which is what splitting it off the major was for, so it refuses nobody and a peer that states none (every build older than the key) reads as the floor. `admit` hands an admitted peer's edition back instead of a bool, and the listener keeps it on the connection's presence entry — a seat that vendors the ledger can then grey a control whose field this engine cannot spell, and render an absent post-floor field as *this engine cannot say* rather than as the reassuring default. |
...
| `src/wire/rendezvous.rs` | **the punched wire's engine end** (REMOTE §13.2–§13.4, bl-4263): the loop's handle and its composition. `Cadence` is every duration the loop keeps — hourly republish, 15 s poll, the punch window, the held connection's quiet — and `Ctx` is what the thread is made of, with the DHT consumed through its `Transport` seam and time through the injected `Clock`, so the suite drives an hour of republishing and a run of polls by advancing a fake clock against the fake DHT the `dht` corpus stands up on loopback. `start` is the engine's call: `None` where `wire/` holds no rendezvous material (REMOTE §13.4's severability — a loopback-only box starts no thread), the loop at the default cadence otherwise. The mainline bootstrap hostnames — the four standard routers, since one of the first two was silent from the deployed engine box (bl-9408) — are stated here and resolved on the loop's own thread, never at boot |
| `src/wire/rendezvous/cycle.rs` | what the thread runs: two deadlines on the clock, and what each does when due. Poll runs before publish when both are due, so the first publish already has a walk behind it. Publish seals the punch port at the advertised addresses plus every address `Dht::observed` names that is not already one of them (the observed ADDRESS only — its port is the DHT socket's UDP mapping, so port preservation is trusted, REMOTE §13.8), signs it under the rendezvous key with a sequence that always moves, and `put`s it; poll `get`s the inbox under the derived key, opens the seal, and punches a call whose nonce is new — on its own thread, every stream that lands handed to the listener's `serve` in its held mode. The DHT client is built on first use once the bootstrap resolves, so an engine that boots offline tries again at the next deadline rather than never. Every outcome is a counter and nothing prints |
| `src/wire/rendezvous/item.rs` | the two sealed items and their byte format, which the client side mirrors (REMOTE §13.2): presence is the engine's endpoint list, a call is a client's nonce and endpoint list, both ChaCha20-Poly1305 under the pairing's derived key with the nonce first, every field fixed-width so a hostile item is bounded before it is read. An item the engine cannot open never draws a SYN |
| `src/wire/rendezvous/material.rs` | the two minted files — `rendezvous.key`, `pairing.salt`, hex — with the three answers `material.rs` gives (nothing, half, whole) and the four HKDF derivations both ends compute from the salt: the two DHT salts, the seal key, and the inbox keypair's seed. Minted by the wire's own mint (`provision::rendezvous_for`) for a stated host, private like a key, never rewritten. Since bl-9043 it also writes `rendezvous.pub` (`publish`, 0644, re-derived wherever absent) and owns the client hand-off: `Handoff` is the (public key, salt) pair the §8.4 envelope answers, `Pairing::handoff` its one derivation, and `bundle` the two file names a client leaf's bundle carries |
| `src/wire/rendezvous/punch.rs` | TCP simultaneous open from one fixed port (REMOTE §13.3): a listener per family on the port and, per target, a third socket bound to the same port that connects, SYNs retried across a bounded window, v6 first. `SO_REUSEADDR` and `SO_REUSEPORT` through `socket2` — the one approved new dependency, REMOTE §13.7 ruling 1. Every stream that lands by a short linger after the first is handed back, because two hosts that can each reach the other's listener form two connections and the engine serves all rather than negotiate. `local_ips` is the route-local address per family, read off a UDP socket that sends nothing |
| `src/wire/server.rs` | the engine's synchronous mTLS listener (REMOTE §4, §8): a non-blocking accept loop so `Drop` stops it, a blocking thread per connection, and the `Answerer` seam the intake fills. An unauthenticated peer fails inside the handshake and never reaches the boundary. Since bl-4263 `serve` takes a `Quiet` — how a connection's silence is read — and a punched stream feeds the same function an accepted one does, in the held mode `peer.rs` describes |
| `src/wire/server/peer.rs` | the peer of one connection (bl-4263): who it is (the leaf's common name and grade), how it is hung up on (half-close and read to EOF, bl-e4c8), and how its silence is read. `Quiet` is the policy: a dialled connection is gone after two minutes of nothing, a held one is pinged through its silence every 25 s (REMOTE §13.4 — a frame, because std has no keepalive and a frame is testable) and gone at the same two minutes. The ping is `{"ping":true}`, never the start of a reply stream, and a client reading a held connection discards it |
...
| `tests/module_reachability.rs` | the orphan guard (bl-862e): every `.rs` under `src/` is declared by a `mod` in its parent module. An undeclared file is not a compile error — it is dead text, and the corpus rule above is what keeps this table from noticing. Stated over the whole of `src/` rather than over test corpora, because "a file with no `mod`" is one rule and `tests/` is not a special case of it. Both directions, like `rules-audit`: the tree must be clean and a fabricated tree with one orphan must still be caught |
...
(written by a child — the ETXTBSY discipline sits at the write, not at the
fork, bl-fd28), fake `/proc` and fake `lsof` output
