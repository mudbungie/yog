...
- **The predicate is brazen's own `credential` column**, the wall's effective
  ...
  Every spelling but `missing` and `not required` readies the wall — `stored`,
...
- **A keyless row is not a credential — unless a role names it.** The ruling's
  ...
  **The clause the door forces (bl-2291):** a `not required` row that the
  ...
  named the row.
- **Two states, not three.** Ready is the fire proceeding, untouched. Refused
  is one sentence — fact first, remedy last: the wall holds no credential, the
  conversation would reach no model, `/login <provider>`, and the rows by name.
  bl-7cc8's third state, Unknown — a wall held elsewhere that this box has not
  read, bl-61bf's seam — dissolves at the door: the engine judges the wall it
  is about to fire into, which is always its own.
...
**What the gate cannot see, recorded rather than hidden:** brazen's *routing*.
It judges the wall and the rows the config points at, never which row a model
prefix resolves to — a role naming an unsigned keyed row beside a signed one is
readied by the signed one, and fails at the first dispatch through §8.3's
auth-shaped step failure and the `failure` clause bl-d774 put on the row,
against a wall that is a fact by then. That is the right side to be wrong on:
a gate that re-derived brazen's route would be a second authority for it. The
same defect one rung over — a bare start into an unsigned wall — is the same
read and the same answer, because there is only one `Prompt` door and one wall
to judge.
