...
| `src/dht/lookup.rs` | the iterative walk: α queries out, one bounded round of answers in, closer nodes learned, repeat until the K closest RESPONSIVE nodes have all been asked (bl-d00f): the frontier is the K closest nodes that replied or are not yet asked, so a node asked and silent, one that answered only an error, and one the socket refused to send to all leave it — and a refused send spends no query and hands its round slot to the next node. A frontier that runs dry short of K replies asks the bootstrap again while the bootstrap has named anyone (the one live router names one random node per query). Bounded twice against a hostile commons — a round ends at its deadline whatever is pending, the walk at `max_queries` however many closer nodes the answers invent. The bootstrap round always asks `find_node` and every later round the walk's own verb, so the BEP 44 verbs reach nodes near the target rather than the routers, which never answer `get` (bl-f6e1). The bootstrap is a door, never a result (bl-9408): its answers seed the pool and its claims vote, but nothing it says is in the outcome, so a walk whose learned nodes were all silent is the dark-commons `Err`, and the pool keys a node by `(distance, address)` so a repeated entry is learned once and one id at two addresses is two nodes. The walk keeps each answering node's `ip` claim for `Dht::observed`; a walk replaces the last one's claims, so a walk that heard nobody observes nothing |
...
| `src/dht/round.rs` | one round, split from `lookup` (bl-d00f): `ask` sends one query and records its transaction only if the send went, `collect` reads answers until every pending transaction has answered or the round's deadline passes, handing on only answers to transactions this round opened. `lookup` walks on them and `items` spends them for the `put` round |
