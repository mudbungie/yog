...
2. Answer the **`Prepared`** (§3.3), carrying the rung's prefill as
   ...
   having none. **A blank goal never sends, and that rule is a seat's**
   (bl-33e9): it is a test of the composer's own text, exactly as the message
   gate's content half is (bl-7cc8), and this crate holds neither the box nor
   any fire path that ever read the predicate once stated here. yog answers the
   prefill; what is typed over it is judged where it is typed. On
