...
| `src/wire/rendezvous/material.rs` | the two minted files — `rendezvous.key`, `pairing.salt`, hex — with the three answers `material.rs` gives (nothing, half, whole) and the four HKDF derivations both ends compute from the salt: the two DHT salts, the seal key, and the inbox keypair's seed. Minted by the wire's own mint (`provision::rendezvous_for`) for a stated host, private like a key, never rewritten |
