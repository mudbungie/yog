...
//! collision under any of the three roots), then `litany new <names-root>/<name>`
...
/// validated name and the `litany` argv sequence (prime, new).
fn new_workspace(
    yog: &TempDir,
    litany_data: &TempDir,
    typed: &str,
) -> Result<(String, Vec<Vec<String>>), NameError> {
    let name = names::validate(typed, &roots(yog.path(), litany_data.path()))?;
    ...
    let argv: Vec<Vec<String>> = litany.invocations().into_iter().map(|i| i.argv).collect();
    Ok((name, argv))
}
...
fn s4_t1_new_workspace_takes_the_operators_typed_name() {
    let (yog, litany_data) = (tempdir().unwrap(), tempdir().unwrap());

    // A lawful name raises the wall: seed first (§8.1 order), then `litany new`
    // at the typed leaf — the pinned template already grants the worker role's
    // whole tool pool, so nothing runs after (§8.1, bl-7fc8).
    let (name, argv) = new_workspace(&yog, &litany_data, "  ops  ").unwrap();
    ...
    for typed in ["", "Ops!", "two words", "-ops", &"a".repeat(33)] {
        assert!(
            new_workspace(&yog, &litany_data, typed).is_err(),
            "{typed:?} is not a lawful sphere label"
        );
    }
    assert_eq!(
        new_workspace(&yog, &litany_data, "unknown").unwrap_err(),
        NameError::Reserved,
    ...
    assert_eq!(
        new_workspace(&yog, &litany_data, "ops").unwrap_err(),
        NameError::Taken("ops".to_owned()),
    );
    // …and so is a name equal to a leaf under **litany's** roots, which are its
    // territory, not yog's, but occupy the same namespace all the same.
    std::fs::create_dir_all(litany_data.path().join("workspaces/acme")).unwrap();
    assert_eq!(
        new_workspace(&yog, &litany_data, "acme").unwrap_err(),
        NameError::Taken("acme".to_owned()),
    );
    // A different name is still free.
    assert!(new_workspace(&yog, &litany_data, "dev").is_ok());
}
