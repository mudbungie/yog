...
//! **without lernie installed**: a real fake-`lernie` recorder script that
//! invokes `$EDITOR` byte-for-byte the way lernie's `edit_in_editor` does
//! (`lernie/src/bin/lernie/cli.rs:22-27`, the task-0 finding) —
...
//! spawns fake-`lernie` with `EDITOR`/`YOG_EDIT_SRC` → fake-`lernie`
//! materializes a checkout, refreshes `descriptions/**` (as lernie does at
//! §3.3), and execs `$EDITOR` → the yog shim copies **only** the staged file
//! over the checkout → fake-`lernie` asserts the staged file landed and its
...
/// Write an executable fake `lernie config` that reproduces lernie's checkout
...
fn write_fake_lernie(dir: &Path) -> std::path::PathBuf {
...
printf 'lernie-refreshed' > "$checkout/descriptions/pool.md"
# lernie's exact $EDITOR hand-off (cli.rs:22-27): word-split editor, one
...
[ "$(cat "$checkout/descriptions/pool.md")" = "lernie-refreshed" ] \
...
fn config_edit_round_trip_through_the_real_shim() {
    ...
    let lernie = write_fake_lernie(dir.path());
    ...
    // exit 0 ⇒ every internal assertion in fake-lernie held: the shim copied
    // the staged file AND lernie's descriptions/** refresh survived.
    assert_eq!(entry.exit, 0, "fake-lernie stderr: {}", entry.stderr);
