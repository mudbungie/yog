        ...
        while let Some((ticket, envelope)) = self.outbox.try_next() {
            ...
            let landed = match codec::decode(&envelope) {
                Ok(Gesture::Act(action)) => {
                    self.acted.push(action.clone());
                    // **Routed, exactly as the poster routes** (REMOTE §8.2):
                    // an act naming a workspace an entry holds is that host's
                    // and never reaches this box's chokepoint. Without this the
                    // fixture would answer every act locally whatever it named,
                    // which is the one thing a beat about a remote start must
                    // not have done for it.
                    if let Some(landed) = self.entry_act(&action) {
                        landed
                    } else {
