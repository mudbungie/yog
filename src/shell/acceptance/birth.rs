...
/// — the two dropdowns over the config-branch head a `lernie prompt` would fork
...
fn a_work_directory_that_is_not_there_flags_red_at_the_field() {
    ...
    let flagged = painted(&mut world, &lernie, &bl);
    assert!(
...
/// default, because lernie takes no per-conversation config.
#[test]
