...
fn balls_section_carries_the_delivered_badge_after_a_close() {
    ...
    assert_eq!(
        ...
        vec![crate::nav::BoundBall {
            ...
            // re-deriving anything from the focus — the project as its §5.1 #1
            // wire name since bl-b4b5, which is the word a `bl` verb takes.
            project: crate::naming::leaf(&w.project),
            owner: "cobalt".to_string(),
            state: JoinState::Delivered,
            // Nothing has been walked, so the figure is the empty one — and it
            // rides the row rather than being asked for per ball (bl-b4b5).
...
fn after_bl_verb_reflects_a_closed_ball_as_delivered() {
    ...
    m.after_bl_verb(&w.project);
    m.tick();
    let delivered = crate::test_support::chrome::ws_balls(&m, &w.ws_cobalt);
...
fn a_names_root_reconcile_rebinds_a_ball_to_a_fresh_workspace() {
    ...
    assert!(
        super::row_for(&m, &newname).is_none(),
    ...
    m.tick();
    let row = super::row_for(&m, &newname).unwrap();
    assert_eq!(
        row.state,
        JoinState::Bound,
