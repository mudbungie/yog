...
/// A `lernie` short verb — names no project, so its effect lands in the yog
/// state root the ops trail is written under.
fn nudge() -> Action {
    Action::Nudge {
...
/// **The aftermath belongs to the receipt.** The root a `lernie` verb touches is
/// marked dirty when the engine says it is done — not at the click, where the
/// act has not happened yet.
#[test]
fn the_root_is_re_derived_when_the_receipt_lands_and_not_before() {
    ...
    assert_eq!(
        ...
        [(h.roots.yog_state.clone(), Mark::Watch)]
            .into_iter()
            .collect(),
        "a lernie verb's ops line lands under the yog state root"
    );
