...
pub(crate) type TailCell = Arc<Mutex<TailSlot>>;
...
pub(crate) struct TailSlot {
    ask: Option<(PathBuf, String)>,
    tail: Option<Arc<crate::app::LiveTail>>,
}

/// The poison-immune guard (see [`lock_cell`] for the discipline).
fn lock_tail(cell: &TailCell) -> MutexGuard<'_, TailSlot> {
    cell.lock().unwrap_or_else(PoisonError::into_inner)
}
...
pub(crate) fn follow(cell: &TailCell, subject: Option<(PathBuf, String)>) {
    let mut slot = lock_tail(cell);
    if slot.ask != subject {
        slot.ask = subject;
    }
}
...
pub(crate) fn asked(cell: &TailCell) -> Option<(PathBuf, String)> {
    lock_tail(cell).ask.clone()
}
...
/// costs zero repaints.
pub(crate) fn publish_tail(cell: &TailCell, tail: Option<crate::app::LiveTail>) -> bool {
    let mut slot = lock_tail(cell);
    if slot.tail.as_deref() == tail.as_ref() {
        return false;
    }
    slot.tail = tail.map(Arc::new);
    true
}

/// The published fold (frame side) — an `Arc` clone, so the frame folds a value
/// nothing can mutate under it.
pub(crate) fn taken_tail(cell: &TailCell) -> Option<Arc<crate::app::LiveTail>> {
    lock_tail(cell).tail.clone()
}
