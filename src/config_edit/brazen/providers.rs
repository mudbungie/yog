...
//! names the row and the `auth` column names its credential model, and both are
//! spelled by brazen's own serde renames.
//!
...
/// One row of the effective provider table (§5.1 #20/#21) — the two columns yog
...
/// Parse `bz --list-providers --json` (`{"providers":[{"name":…,"auth":…},…]}`)
/// into rows, in listing order — which IS brazen's routing order. A non-object
/// or shapeless payload folds to no rows, never an error; a row missing either
/// column folds to an empty string for it (an unnamed row is unroutable and an
/// unknown auth model is not `oauth2`, so both degrade to "not loginable"
/// without a branch).
