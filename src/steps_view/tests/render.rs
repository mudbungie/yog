...
//!
//! This file is the **list**: the table, its badges, its columns and its
//! figures. The drill-in's half is [`drill`](super::drill), split off at §12's
//! budget on the seam the production paints are already cut on (`render` the
//! table, `drill` the record trees below it) — and it renders through the
//! [`painted`] and [`summary`] fixtures here, so there is one of each.

use std::collections::HashSet;

use crate::git_tree::Framing;
use crate::steps_view::render::{StepTab, render};
use crate::steps_view::{Orphan, StepDetail, StepSummary, StepsView, Wound};
...
pub(super) fn summary(seq: &str, framing: Framing) -> StepSummary {
    StepSummary {
        seq: seq.into(),
        framing,
        attempts: 2,
