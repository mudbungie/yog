...
mod bl_tracker {
    ...
    pub(super) fn run(args: &[String]) -> i32 {
        ...
        // `Env::resolve`, never the struct literal: the chain PARSE is balls'
        // (an absent variable fails OPEN as an empty chain, publishing), and
        // `$BALLS_HELD_STORES` is the chain of store checkouts core stamps on
        // every plugin spawn — the fact that tells this tracker whether an
        // enclosing `bl` still holds THIS store open (balls 0.5.12, upstream
        // bl-aac7; it replaced the store-blind depth read of bl-1266).
