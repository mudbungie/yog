...
//! is a sliding window, not lockstep rounds (bl-d9c1): up to α queries in the
...
impl Dht {
    ...
    /// (REMOTE §13.7 ruling 3), the one router that answers names a single
    ...
    ///
    /// A re-ask knocks only at the bootstrap addresses that answered
