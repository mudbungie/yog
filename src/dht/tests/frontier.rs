//! The walk's frontier (bl-d00f): a silent node leaves it, a node the socket
...
/// dark-commons `Err` (measured against the selection before bl-d00f).
#[test]
...
/// The live dark walk's shape (bl-f519): of the bootstrap addresses, one is
/// silent every time and one is the rotor naming a single node per answer —
