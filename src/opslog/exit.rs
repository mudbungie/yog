...
impl OpRow {
    ...
    /// carries stderr — which for a `-2` line can only be the driver's own
    /// dying words, folded in at read time from its per-spawn sink
    /// ([`super::detached`]), since a spawn that never launched no longer writes
    /// one (it is [`ExitKind::NeverSpawned`]). Everything else — a non-zero
    /// code, a signal, a never-started spawn, a failed step — is.
    ...
    /// [`failed`](Self::failed) — a `-2` row with a folded stderr is already a
    /// failure and never reaches this arm — so the two are mutually exclusive.
    /// The §11 rollup layer ([`super::live::outcomes`]) reads this, never the
    /// raw `-2` sentinel, to classify the row `OpOutcome::Detached` rather than
    /// `Clean`: nobody observed the exit, so it must not read as one.
