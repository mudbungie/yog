...
fn a_pick_on_a_row_brazen_lacks_is_refused_before_either_file() {
    ...
    let refused = plan(SEEDED_MODELS, TEMPLATE_PROVIDERS, &live, &pick("gpt-5.4"));
    assert_eq!(
        refused,
...
fn a_model_id_the_block_grammar_cannot_hold_is_refused() {
    for bad in ["", "   ", "gpt 5", "gpt:5", "gpt#5", "a\tb"] {
        ...
        assert_eq!(
            plan(SEEDED_MODELS, TEMPLATE_PROVIDERS, &rows(), &broken),
...
fn an_unanswerable_brazen_gates_no_pick() {
    assert!(plan(SEEDED_MODELS, TEMPLATE_PROVIDERS, &[], &pick("gpt-5.4")).is_ok());
}
