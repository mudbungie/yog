...
fn a_punch_lands_on_the_peers_listener_and_carries_bytes() {
    ...
    let served = std::thread::spawn(move || {
        let mut streams = engine.punch(vec![], Duration::from_secs(5));
        let mut byte = [0u8; 1];
        streams
            .first_mut()
            .expect("accepted")
    ...
    let mut streams = client.punch(vec![target], Duration::from_secs(5));
    let stream = streams.first_mut().expect("connected");
    assert_eq!(
...
fn two_ends_punching_each_other_both_hold_a_connection() {
    ...
    let from_a = from_a.join().expect("a");
    assert!(!from_a.is_empty() && !from_b.is_empty());
    assert_eq!(from_a.len(), from_b.len(), "every connection has two ends");
}
...
fn a_port_nothing_can_bind_refuses() {
    let refusal = Punch::bind(1).err().expect("privileged");
...
fn a_v6_loopback_punch_lands_where_the_box_has_v6() {
    ...
    let client = Punch::bind(0).expect("bind");
    let served = std::thread::spawn(move || engine.punch(vec![], Duration::from_millis(600)).len());
    let landed = client.punch(vec![target], Duration::from_millis(500)).len();
    assert_eq!(
        landed == 1,
        has_v6,
        "a v6 SYN lands exactly where a v6 listener is"
    );
    assert_eq!(served.join().expect("served") == 1, has_v6);
}
