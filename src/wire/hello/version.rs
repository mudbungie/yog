//! **The wire version** (REMOTE §3): the changelog of what moved the integer
//! each end writes in its preface. Split off [`super`] at §12's cap.
//!
...
//!
//! **A second integer IS declared here** (bl-9ced): the newest version yog has
//! PUBLISHED. Nothing outside this tree fetches it — its one reader is the
//! corpus ledger's gate, an ordinary unit test — so it needs no address of its
//! own, and it is stated rather than derived because that reader runs on a
//! bare checkout. The pair is the two halves of the `<n> → <m>` heading every
//! entry below already writes in prose.
//!
