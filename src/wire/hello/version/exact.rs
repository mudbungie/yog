//! **The exact era of the wire version, closed** (bl-e598): every entry the
//! ledger wrote while `PROTOCOL` moved on any wire-visible change, one integer
//! per change, 4 → 18. Moved here whole from [`super`] at §12's cap, which is
//! the seam the ledger named for itself: no entry is ever deleted, so the
//! pre-split band is answered by an era and not by trimming prose.
//!
//! Read it as the record of what each **edition** 4..18 in `corpus/shapes.json`
//! meant — the stamps are this line continued — and as the evidence for the
//! ruling that closed it: of the five bumps taken in the week 13 → 18, four
//! carried nothing but additions, and the fifth carried one required request
//! field that REMOTE §3.2's default-absent discipline would have made
//! additive too. Nothing in this file is read by a program.
//!
...
//! for and what nothing could honour until [`super::PROTOCOL_PUBLISHED`]: the
