...
pub(super) fn push_entry(
    ...
    match &entry.kind {
        ...
        // The live tail is up to **two** rows and they are the same two a
        // committed model turn has — reasoning, then the answer (§7.2 the
        // thinking ruling). Each keeps its committed counterpart's class, so
        // the fold knobs mean one thing on either side of the commit; what
        // differs is the tone, and `Tone::Live` is what auto-expands them
        // while the step is happening (`super::expanded_for`). An empty half
        // is no row at all: a model that has only thought so far shows one
        // growing row, not one growing row and one blank one.
