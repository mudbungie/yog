//! **The row that stands for what is NOT in the record** (bl-7bd2) — the
//! compaction marker, split off [`super`] at §12's budget on the seam that
//! module's own match already draws: every other arm there projects something
//! somebody said or a tool answered, and this one projects a *hole*.
//!
//! It is the row altitude of [`crate::transcript::compaction`], which is the
//! enumeration's one reader of what the compactor deleted — so the two halves
//! of that one subject sit one per file rather than one per subsystem.
...
const COMPACTED_HOVER: &str = "These entries were removed: litany's compactor squashed them out of the record and \
...
/// faded line stating what is missing, folding open onto what litany put in
